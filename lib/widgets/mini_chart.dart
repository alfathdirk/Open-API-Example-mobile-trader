...
import 'package:myfxhistory_app/models/activity_manager.dart';
...
class _MiniChartState extends State<MiniChart> {
  ...
  void initState() {
    ...
    trader.positionsManager
        .addListener(ActivityManagerEventType.added, _onPositionCreatedEvent);
    trader.positionsManager
        .addListener(ActivityManagerEventType.removed, _onPositionRemovedEvent);
    trader.positionsManager
        .addListener(ActivityManagerEventType.updated, _onPositionUpdated);
    trader.ordersManager
        .addListener(ActivityManagerEventType.added, _onOrderCreatedEvent);
    trader.ordersManager
        .addListener(ActivityManagerEventType.removed, _onOrderRemovedEvent);
    trader.ordersManager
        .addListener(ActivityManagerEventType.updated, _onOrderUpdatedEvent);
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
          ...
          onPageFinished: (String url) {
            _chartLoaded = true;
            final String bg =
                THEME.background().value.toRadixString(16).substring(2);
            final String fg =
                THEME.onBackground().value.toRadixString(16).substring(2);
            _symbolId = widget.symbol.id;
            _controller.runJavaScript(
                "initChart('#$bg', '#$fg', '${widget.symbol.name}', ${widget.showToolbar});chart.setSymbol('${widget.symbol.name}');");
          },
          ...
          onNavigationRequest: (NavigationRequest request) {
            if (request.url.startsWith('file://')) {
              return Future<NavigationDecision>.value(
  ...
  void dispose() {
    ...
    if (userState.selectedTraderId > 0) {
      try {
        ...
        if (_period != null)
          trader.subscriptionManagerApi
              .unsubscribeFromTrendBars(trader.id, _symbolId, _period!);

        trader.positionsManager.removeListener(
            ActivityManagerEventType.added, _onPositionCreatedEvent);
        trader.positionsManager.removeListener(
            ActivityManagerEventType.removed, _onPositionRemovedEvent);
        trader.positionsManager.removeListener(
            ActivityManagerEventType.updated, _onPositionUpdated);
      } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.showToolbar != null && widget.showToolbar != _showToolbar) {
      ...
      _controller.runJavaScript(
          "document.getElementsByTagName('body')[0].className='${_showToolbar == true ? "" : "portreit"}';");
    }
    ...
    return Stack(children: <Widget>[
      ...
      if (widget.showRotNotif == true)
        Positioned.fill(child: Container(color: THEME.chartBlockBackground())),
      if (widget.showRotNotif == true)
        Positioned(
          right: 8,
          top: 8,
        ...
        Positioned.fill(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Text(l10n.chartRotateTip,
                  style: THEME.texts.chartRotateTip,
  ...
  Future<void> _loadCandlesToChart(
      String jsCallback, String timeframe, int count,
      [int? tsFrom]) async {
    final TraderData trader = GetIt.I<UserState>().selectedTrader;
    final RemoteApi remoteAPI = trader.remoteApi;
    final SpotSubscriptionManager spotSubscriptionManager =
        trader.subscriptionManagerApi;
    final ProtoOATrendbarPeriod period =
        ProtoOATrendbarPeriodExtention.byChartName(timeframe);

    count = min(count, 250);

    Logger.debug(() =>
        'MiniChart: chart requested to load candles: $timeframe, $count, $tsFrom');
    ...
    if (_period != null && _period != period) {
      try {
        await spotSubscriptionManager.unsubscribeFromTrendBars(
            trader.id, _symbolId, _period!);
      } catch (e) {
        Logger.log(() =>
            'Error occurred at unsubscribing from trndbars for symbol($_symbolId) of ${_period!.name}(${_period!.index})');
      }
    }

    final int to =
        tsFrom != null ? tsFrom * 1000 : DateTime.now().millisecondsSinceEpoch;
    final int from = DateTime.fromMillisecondsSinceEpoch(to)
        .subtract(Duration(seconds: period.seconds() * count))
        .millisecondsSinceEpoch;

    // need to wait until symbol subscribded for spots
    while (
        !spotSubscriptionManager.isSymbolSubscribedForSpots(widget.symbol.id)) {
    ...
    if (_period == null || _period != period) {
      ...
      try {
        _period = period;
        await spotSubscriptionManager.subscribeForTrendBars(
            trader.id, widget.symbol.id, period);
      } catch (err) {
        _period = null;
    ...
    try {
      trendbarsRes = await remoteAPI.sendGetTrendbars(
          trader.id, widget.symbol.id, count, period, from, to);
    } catch (err) {
      Logger.error('Error occurred at loading candles for chart', err);
      String msg = "Can't load trendbars history";
      if (err is ProtoOAErrorRes) {
        msg += AppLocalizations.of(context)!
                .getServerErrorDescription(err.errorCode) ??
            err.description ??
            err.errorCode;
      } else {
        msg += '$err';
    ...
    for (final ProtoOATrendbar bar in trendbarsRes.trendbar) {
      candles.add(<String, dynamic>{
        ...
        'open': SymbolData.humanicRateFromSystem(
            (bar.low ?? 0) + (bar.deltaOpen ?? 0)),
        'high': SymbolData.humanicRateFromSystem(
            (bar.low ?? 0) + (bar.deltaHigh ?? 0)),
        'low': SymbolData.humanicRateFromSystem(bar.low ?? 0),
        'close': SymbolData.humanicRateFromSystem(
            (bar.low ?? 0) + (bar.deltaClose ?? 0)),
        'volume': bar.volume,
      });
    }
  ...
  void _addPositionsToChart(TraderData trader) {
    final Iterable<PositionData> positions =
        trader.positionsManager.activitiesBy(symbolId: widget.symbol.id);
    for (final PositionData p in positions) {
      final double rate = SymbolData.humanicRateFromSystem(p.rate);
      final double? tp = p.takeProfit != null
          ? SymbolData.humanicRateFromSystem(p.takeProfit!)
          : null;
      final double? sl = p.stopLoss != null
          ? SymbolData.humanicRateFromSystem(p.stopLoss!)
          : null;
  ...
  void _addOrdersToChart(TraderData trader) {
    final Iterable<OrderData> orders =
        trader.ordersManager.activitiesBy(symbolId: widget.symbol.id);
  ...
  Future<void> _onHandleSpotEvent(ProtoOASpotEvent event) async {
    if (_chartLoaded && event.symbolId == widget.symbol.id) {
      ...
      if (_period != null &&
          event.trendbar != null &&
          event.trendbar!.isNotEmpty) {
        final ProtoOATrendbar trendbar = event.trendbar!.first;
        if (_period == trendbar.period) {
          final DateTime ts =
              DateTime.utc(1970, 1, 1, 0, trendbar.utcTimestampInMinutes!);
          final String msg =
              "chart.data.setCandle('${widget.symbol.name}', '${_period!.toChartName()}',"
  ...
  Future<void> _symbolChanged() async {
    ...
    if (_period != null) {
      GetIt.I<UserState>()
          ...
          .unsubscribeFromTrendBars(
              GetIt.I<UserState>().selectedTraderId, _symbolId, _period!);
    }
    ...
    await _controller.runJavaScript(
        "dropAllDeals(); chart.setSymbol('${widget.symbol.name}');");
  }
  ...
  void _onPositionCreatedEvent(PositionData position) {
    ...
    final double? tp = position.takeProfit != null
        ? SymbolData.humanicRateFromSystem(position.takeProfit!)
        : null;
    final double? sl = position.stopLoss != null
        ? SymbolData.humanicRateFromSystem(position.stopLoss!)
        : null;
  ...
  void _onOrderUpdatedEvent(OrderData order) {
    ...
    final String stopLossRate = order.stopLoss != null
        ? '"${SymbolData.humanicRateFromSystem(order.stopLoss!)}"'
        : 'null';
  ...
  Future<void> _updateDealSLTPByChartEvent(int positionId,
      {double? takeProfit, double? stopLoss}) async {
    final TraderData trader = widget.symbol.trader;
    final PositionData? position =
        trader.positionsManager.activityBy(id: positionId);
    ...
    if (position.symbol?.details?.data.tradingMode ==
            ProtoOATradingMode.disabledWithPendingsExecution ||
        position.symbol?.details?.data.tradingMode ==
    ...
    final int rounder =
        pow(10, 5 - (symbol?.details?.data.digits ?? 0)).toInt();
    ...
    if (takeProfit != null) {
      if (takeProfit < 0) {
        ...
        if (takeProfitDisatance != null && rate != null) {
          final int extremeRate =
              rate + takeProfitDisatance * (position.isBuy ? 1 : -1);
          if ((position.isBuy && tp < extremeRate) ||
              (!position.isBuy && tp > extremeRate)) {
            tp = null;
          }
        }
      }
    ...
    if (stopLoss != null) {
      if (stopLoss < 0) {
        ...
        sl = (SymbolData.systemRateFromHumanic(stopLoss) / rounder).round() *
            rounder;
        final int? rate = position.currentRate;
        ...
        if (rate != null && lossDistance != null) {
          final int extremeRate =
              rate + lossDistance * (position.isBuy ? -1 : 1);
          if ((position.isBuy && sl > extremeRate) ||
              (!position.isBuy && sl < extremeRate)) {
            sl = null;
          }
        }
      }
  ...
  Future<void> _updateOrderSLTPByChartEvent(int orderId,
      {double? takeProfit, double? stopLoss}) async {
    if (!mounted) return;
    ...
    if (order.symbol?.details?.data.tradingMode ==
            ProtoOATradingMode.disabledWithPendingsExecution ||
        order.symbol?.details?.data.tradingMode ==
            ProtoOATradingMode.disabledWithoutPendingsExecution) {
      GetIt.I<PopupManager>()
          .showSymbolDisabledByTradingMode(AppLocalizations.of(context)!);
    }
    ...
    if (takeProfit != null) {
      if (takeProfit < 0) {
        ...
        if (takeProfitDisatance != null) {
          final int extremeRate =
              order.rate + takeProfitDisatance * (order.isBuy ? 1 : -1);
          if ((order.isBuy && tp < extremeRate) ||
              (!order.isBuy && tp > extremeRate)) {
            tp = null;
          }
        }
      }
    ...
    if (stopLoss != null) {
      if (stopLoss < 0) {
        ...
        if (lossDistance != null) {
          final int extremeRate =
              order.rate + lossDistance * (order.isBuy ? -1 : 1);
          if ((order.isBuy && sl > extremeRate) ||
              (!order.isBuy && sl < extremeRate)) {
            sl = null;
          }
        }
      }
    ...
    if (order.stopLoss == sl && order.takeProfit == tp) {
      ...
      await order.editOrder(context, order.rate, order.volume,
          takeProfit: tp,
          stopLoss: sl,
          expiresAtTs: order.expireAt?.millisecondsSinceEpoch);
    }
  }
  ...
  Future<void> _closePositionByChartEvent(int positionId) async {
    ...
    if (trader.accessRights.index >= ProtoOAAccessRights.noTrading.index) {
      ...
      if (position?.symbol?.details?.data.tradingMode ==
        ...
        try {
          await trader.positionsManager
              .activityBy(id: positionId)
