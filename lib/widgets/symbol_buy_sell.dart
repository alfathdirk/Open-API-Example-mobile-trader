...
import 'package:myfxhistory_app/models/symbol_icons.dart';
...
import 'package:myfxhistory_app/screens/edit_order_position.dart';
import 'package:myfxhistory_app/states/app_state.dart';
import 'package:myfxhistory_app/states/user_state.dart';
import 'package:myfxhistory_app/widgets/buy_sell_button.dart';
...
class _SymbolBuySellState extends State<SymbolBuySell> {
  ...
  Widget build(BuildContext context) {
    ...
    final Color dcColor =
        (widget.symbol?.dailyChangePips ?? 0) < 0 ? THEME.red : THEME.green;
          ...
          Container(
            decoration: BoxDecoration(
              ...
              color: widget.isSelected == true
                  ? THEME.marketsSymbolSelectedBackground()
                  : null,
            ),
                            ...
                            children: <Widget>[
                              ...
                              Expanded(
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: <Widget>[
                                    Text(widget.symbol?.name ?? 'n/a',
                                        style: THEME.texts.headingBold,
                                        overflow: TextOverflow.ellipsis),
                                    Text(
                                        widget.symbol?.category.assetClass
                                                .name ??
                                            'n/a',
                                        style:
                                            THEME.texts.bodyRegularSecondary),
                                  ],
                                ),
                              ),
                              ...
                              Column(
                                  ...
                                  children: <Widget>[
                                    Text(
                                      widget.symbol
                                              ?.formattedDailyChangePercents ??
                                          ' n/a',
                                      style: THEME.texts.headingBold
                                          .copyWith(color: dcColor),
                                    ),
                                    Text(
                ...
                AnimatedSize(
                  ...
                  child: Column(children: <Widget>[
                    if (widget.symbol != null)
                      for (final PositionData pos in trader.positionsManager
                          .activitiesBy(symbolId: widget.symbol!.id))
                        _positionListItem(l10n, pos),
                    if (widget.symbol != null)
                      for (final OrderData order in trader.ordersManager
                          .activitiesBy(symbolId: widget.symbol!.id))
                        _orderListItem(l10n, order),
                  ]),
                ),
            ]),
          ),
          if (widget.symbol?.isMarketClosed() == true)
            _blockWithTraidingHoursOverlay(l10n),
        ],
      ),
    );
  }
      ...
      child: Container(
        ...
        child: Column(
            ...
            children: <Widget>[
              ...
              Text(l10n.tradingForSymbolClosed,
                  style: THEME.texts.headingLargeBold),
            ]),
      ),
    );
  }
    ...
    return Container(
      ...
      child: details == null
          ...
          : Column(children: <Widget>[
              ...
              for (final Widget item in _symbolDetailsLeverage(
                  l10n, widget.symbol?.details?.leverages))
                item,
              for (final Widget item
                  in _symbolDetailsSwap(l10n, widget.symbol?.details))
                item,
              if (details.minVolume != null)
                _symbolMinimumOrder(l10n, details.minVolume, style),
              if (details.maxVolume != null)
                _symbolMaximumOrder(l10n, details.maxVolume, style),
              for (final Widget item
                  in _symbolDetailsCommissions(l10n, widget.symbol?.details))
                item,
              const SizedBox(height: 12),
  ...
  List<Widget> _symbolDetailsLeverage(
      AppLocalizations l10n, List<ProtoOADynamicLeverageTier>? leverages) {
    final List<Widget> items = <Widget>[];
    final TextStyle style = THEME.texts.bodyMedium;
    final int traderLeverage =
        GetIt.I<UserState>().selectedTrader.levelrageInCents;
    ...
    if (leverages == null || leverages.isEmpty) {
      ...
      items.add(Row(children: <Widget>[
        ...
        Text('1:', style: style),
        SizedBox(
            width: _lastColumnWidth, child: Text('$leverage', style: style)),
      ]));
    } else {
        ...
        child: Column(children: <Widget>[
          ...
          Row(children: <Widget>[
            ...
            Container(
                ...
                alignment: Alignment.center,
                child: Text(l10n.leverage, style: style)),
          ]),
          for (int i = 0; i < leverages.length; i++)
            Row(children: <Widget>[
              ...
              SizedBox(
                  ...
                  child: Text(
                      '${min(leverages[i].leverage, traderLeverage) ~/ 100}',
                      style: style)),
            ])
        ]),
      ));
  ...
  List<Widget> _symbolDetailsSwap(
      AppLocalizations l10n, SymbolDetailsData? details) {
    final List<Widget> items = <Widget>[];
    final TextStyle style = THEME.texts.bodyMedium;

    if (details != null) {
      if (details.symbol.trader.swapFree) {
        if (details.data.rolloverCommission != null &&
            details.data.rolloverCommission! > 0) {
          if (details.data.rolloverCommission3Days != null) {
            items.add(Row(children: <Widget>[
              ...
              SizedBox(
                width: _lastColumnWidth * 2,
                child: Text(
                    l10n.dayOfWeek(details.data.rolloverCommission3Days!.name)),
              ),
            ]));
          }
        ...
        if (details.data.swapRollover3Days != null &&
            details.data.swapRollover3Days != ProtoOADayOfWeek.none) {
          items.add(Row(children: <Widget>[
            Text(l10n.swap3Days, style: style),
            const Spacer(),
            SizedBox(
              width: _lastColumnWidth * 2,
              child: Text(l10n.dayOfWeek(details.data.swapRollover3Days!.name),
        ...
        if (details.data.swapLong != null) {
          ...
          items.add(Row(children: <Widget>[
            Text(
                l10n.swapPerLot(value.toComaSeparated(),
                    details.data.measurementUnits ?? '', 'long'),
                style: style),
            const Spacer(),
            ...
            ConstrainedBox(
                ...
                child: Text(
                  details.data.swapCalculationType ==
        ...
        if (details.data.swapShort != null) {
          ...
          items.add(Row(children: <Widget>[
            Text(
                l10n.swapPerLot(value.toComaSeparated(),
                    details.data.measurementUnits ?? '', 'short'),
                style: style),
            const Spacer(),
  ...
  Widget _symbolMinimumOrder(
      AppLocalizations l10n, int volume, TextStyle style) {
    return Row(children: <Widget>[
      ...
      ConstrainedBox(
        constraints: const BoxConstraints(minWidth: _lastColumnWidth),
        child: Text(' ${widget.symbol?.units ?? ''}',
            style: style, softWrap: false),
      ),
    ]);
  }

  Widget _symbolMaximumOrder(
      AppLocalizations l10n, int volume, TextStyle style) {
    return Row(children: <Widget>[
      Text('${l10n.maxOrder}:', style: style),
      const Spacer(),
      Text((volume / 100).toComaSeparated(decimals: volume % 100 == 0 ? 0 : 2),
          style: style),
      ConstrainedBox(
        constraints: const BoxConstraints(minWidth: _lastColumnWidth),
        child: Text(' ${widget.symbol?.units ?? ''}',
  ...
  List<Widget> _symbolDetailsCommissions(
      AppLocalizations l10n, SymbolDetailsData? details) {
    final List<Widget> items = <Widget>[];
    final TextStyle style = THEME.texts.bodyMedium;
    ...
    if (preciseMinCommission != null && preciseMinCommission > 0) {
      final String currency = details?.data.minCommissionType ==
      ...
      items.add(Row(children: <Widget>[
        ...
        const Spacer(),
        Text((preciseMinCommission / 100000000).toComaSeparated(decimals: 2),
            style: style),
        SizedBox(
          width: _lastColumnWidth,
    ...
    if (commissionRate != null && commissionRate > 0) {
      ...
      if (details?.data.commissionType ==
          ProtoOACommissionType.percentageOfValue) {
        value.write(l10n
            .commissionPercentageOfValue((commissionRate / 100000).toString()));
      } else {
        final double amount = commissionRate / 100000000;
        if (details?.data.commissionType ==
            ProtoOACommissionType.usdPerMillionUsd) {
          value.write(
              '${amount.toComaSeparated(decimals: 2)} ${l10n.commissionUsdPerMillionUsd}');
        } else if (details?.data.commissionType ==
            ProtoOACommissionType.usdPerLot) {
          ...
          final AssetData? asset =
              details?.symbol.tree.asset(details.symbol.quoteAssetId);
  ...
  Widget _symbolDetailsTradingHours(
      AppLocalizations l10n, SymbolDetailsData? details) {
    final TextStyle style = THEME.texts.bodyMedium;
    final List<ProtoOAInterval> schedule =
        details?.data.schedule ?? <ProtoOAInterval>[];
    final List<Widget> items = <Widget>[];
    final tz.Location tzLocation =
        tz.getLocation(details?.data.scheduleTimeZone ?? 'UTC');
    ...
    for (final ProtoOAInterval interval in schedule) {
      final tz.TZDateTime start = tz.TZDateTime(tzLocation, sunday.year,
              sunday.month, sunday.day, 0, 0, interval.startSecond)
          .toLocal();
      final tz.TZDateTime end = tz.TZDateTime(tzLocation, sunday.year,
              sunday.month, sunday.day, 0, 0, interval.endSecond)
          .toLocal();
    ...
    final List<ProtoOAHoliday> holidays =
        details?.holidays ?? <ProtoOAHoliday>[];
    ...
    if (holidays.isNotEmpty) {
      ...
      if (holidays.length > 1 &&
          holidays[0].startDateTime().difference(DateTime.now()).inDays < 7)
  ...
  List<Widget> _symbolDetailsHoliday(
      AppLocalizations l10n, ProtoOAHoliday holiday, bool isNext) {
    final List<Widget> items = <Widget>[];
    ...
    items.add(Row(children: <Widget>[
      const SizedBox(width: 16),
      Expanded(
          child: Text('${isNext ? l10n.nextHoliday : l10n.holiday}:',
              style: THEME.texts.bodyBold)),
      Expanded(
          child: Text(holiday.name,
              style: THEME.texts.bodyBold, overflow: TextOverflow.ellipsis)),
      const SizedBox(width: 16),
    ]));
    ...
    if (starts.day == ends.day) {
      items.add(Row(children: <Widget>[
        ...
        Expanded(
            child:
                Text(starts.formatted('MMM d'), style: THEME.texts.bodyMedium)),
        const SizedBox(width: 16),
      ]));
      items.add(Row(children: <Widget>[
        const SizedBox(width: 16),
        ...
        Expanded(
          child: Row(
            children: <Widget>[
              Expanded(
                  child: Text(starts.formatted('HH:mm'),
                      style: THEME.texts.bodyMedium)),
              Expanded(
                  child: Text('${l10n.to.toLowerCase()}:',
                      style: THEME.texts.bodyMedium,
                      textAlign: TextAlign.center)),
              Expanded(
                  child: Text(ends.formatted('HH:mm'),
                      style: THEME.texts.bodyMedium,
                      textAlign: TextAlign.right)),
            ],
          ),
      ...
      items.add(Row(children: <Widget>[
        const SizedBox(width: 16),
        Expanded(
            child: Text('${l10n.dateFrom}:', style: THEME.texts.bodyMedium)),
        Expanded(
          child: Row(
            children: <Widget>[
              Expanded(
                  child: Text(starts.formatted('MMM d'),
                      style: THEME.texts.bodyMedium)),
              Expanded(
                  child: Text(starts.formatted('HH:mm'),
                      style: THEME.texts.bodyMedium)),
      ...
      items.add(Row(children: <Widget>[
        ...
        Expanded(
          child: Row(
            children: <Widget>[
              Expanded(
                  child: Text(ends.formatted('MMM d'),
                      style: THEME.texts.bodyMedium)),
              Expanded(
                  child: Text(ends.formatted('HH:mm'),
                      style: THEME.texts.bodyMedium)),
  ...
  Widget _positionListItem(AppLocalizations l10n, PositionData position) {
    ...
    return IntrinsicHeight(
      child: Container(
        ...
        decoration: BoxDecoration(
            border: Border(top: BorderSide(color: THEME.dividerLight()))),
        child: Flex(direction: Axis.horizontal, children: <Widget>[
          Container(
            ...
            child: SvgPicture.asset(
              GetIt.I<SymbolIcons>()
                  .getIconNameBySymbolName(widget.symbol?.name),
              width: 27,
              height: 27,
            ),
          ),
          ...
          Expanded(
            child: FittedBox(
              ...
              child: Text(position.formattedVolumeWithUnits,
                  style: THEME.texts.bodyMedium),
            ),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: FittedBox(
              ...
              child: Text(
                GetIt.I<UserState>()
                    .selectedTrader
                    .formattedMoneyWithCurrency(cents: position.netPnl),
        ...
        child: Flex(direction: Axis.horizontal, children: <Widget>[
          ...
          Expanded(
            child: FittedBox(
              alignment: Alignment.centerLeft,
              fit: BoxFit.scaleDown,
              child: Text(order.formattedVolumeWithUnits,
                  style: THEME.texts.bodyMedium),
            ),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: FittedBox(
              alignment: Alignment.centerRight,
              fit: BoxFit.scaleDown,
              child: Text('@${order.formattedRate(system: order.rate)}',
                  style: THEME.texts.bodyMedium),
            ),
          ),
          const SizedBox(width: 8),
  ...
  void _toggleFavorite() {
    if (widget.symbol != null)
      GetIt.I<UserState>().toggleFavoriteSymbol(widget.symbol!.id);
  ...
  Future<void> _toggleShowDetails() async {
    ...
    if (_showDetails && widget.symbol!.details == null) {
      await remoteApi
          .sendSymbolById(widget.symbol!.trader.id, <int>[widget.symbol!.id]);
    }

    final int? leverageId = widget.symbol!.details?.data.leverageId;
    if (_showDetails &&
        leverageId != null &&
        !widget.symbol!.tree.hasLeverage(leverageId)) {
      remoteApi.sendLeverageById(widget.symbol!.trader.id, leverageId).then(
          (ProtoOAGetDynamicLeverageByIDRes resp) =>
              widget.symbol!.tree.addLeverage(resp.leverage));
    }
  ...
  void _onTapEditPosition(int positionId) {
    if (GetIt.I<UserState>().selectedTrader.accessRights.index >
        ProtoOAAccessRights.closeOnly.index) {
      final UserState userState = GetIt.I<UserState>();
      GetIt.I<PopupManager>().showTraderSelection(AppLocalizations.of(context)!,
          userState.selectedTrader, userState.traders.length > 1);
    } else if (widget.symbol?.details?.data.tradingMode ==
            ProtoOATradingMode.disabledWithPendingsExecution ||
        widget.symbol?.details?.data.tradingMode ==
            ProtoOATradingMode.disabledWithoutPendingsExecution) {
      GetIt.I<PopupManager>()
          .showSymbolDisabledByTradingMode(AppLocalizations.of(context)!);
    } else {
      if (widget.onSelect != null) widget.onSelect!();
      Navigator.pushNamed(context, EditOrderPositionScreen.ROUTE_NAME,
          arguments: <String, dynamic>{'positionId': positionId});
    }
  }

  Future<void> _closePosition(
      AppLocalizations l10n, PositionData position) async {
    if (GetIt.I<UserState>().selectedTrader.accessRights.index >
        ProtoOAAccessRights.closeOnly.index) {
      final UserState userState = GetIt.I<UserState>();
      GetIt.I<PopupManager>().showTraderSelection(AppLocalizations.of(context)!,
  ...
  Future<void> _cancelOrder(AppLocalizations l10n, int orderId) async {
    if (GetIt.I<UserState>().selectedTrader.accessRights.index >
      ...
      GetIt.I<AppState>().setUIBlocked(true);
      await GetIt.I<UserState>()
          .selectedTrader
          .ordersManager
          .activityBy(id: orderId)
          ?.cancelOrder(context);
      GetIt.I<AppState>().setUIBlocked(false);
    }
  }
