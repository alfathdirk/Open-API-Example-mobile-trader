...
class SideMenu extends StatelessWidget {
      ...
      child: SafeArea(
        child: Column(
          children: <Widget>[
            ...
            _menuItem(l10n.myActivity, () => _openMyActivity(context),
                widgets: <Widget>[
                  _activityCircleAmount(
                    userState.selectedTrader.pnl < 0 ? THEME.red : THEME.green,
                    userState.selectedTrader.positionsManager.count,
                  ),
                  _activityCircleAmount(THEME.menuActivityOrders(),
                      userState.selectedTrader.ordersManager.count),
                ]),
        ...
        children: <Widget>[
          Expanded(
              child: Text(l10n.menu,
                  style: THEME.texts.sideMenu, textAlign: TextAlign.center)),
          GestureDetector(
      ...
      child: Container(
        ...
        child: FittedBox(
            child: Text(amount.toString(),
                style: THEME.texts.sideMenuActivityCircles)),
      ),
    );
  }
  ...
  void _openMarkets(BuildContext context) {
    ...
    Navigator.pushNamedAndRemoveUntil(
        context, MarketScreen.ROUTE_NAME, (Route<dynamic> route) => false);
  }
  ...
  void _openMyAccount(BuildContext context) {
    ...
    Navigator.pushNamedAndRemoveUntil(
        context, AccountScreen.ROUTE_NAME, (Route<dynamic> route) => false);
  }
  ...
  void _openMyActivity(BuildContext context) {
    ...
    Navigator.pushNamedAndRemoveUntil(
        context, ActivityScreen.ROUTE_NAME, (Route<dynamic> route) => false);
  }
  ...
  Future<void> _openTradingRoom(BuildContext context) async {
    ...
    if (!GetIt.I<AppState>().isTrTermsChecked) {
      final List<dynamic> json =
          jsonDecode(await rootBundle.loadString('assets/json/tr_terms.json'))
              as List<dynamic>;
      final Iterable<List<String>> terms =
          json.cast<List<dynamic>>().map((List<dynamic> e) => e.cast<String>());
      final bool? agree = await Navigator.push<bool?>(
          context,
    ...
    Navigator.pushNamedAndRemoveUntil(
        context, TradingRoomScreen.ROUTE_NAME, (Route<dynamic> route) => false);
  }
