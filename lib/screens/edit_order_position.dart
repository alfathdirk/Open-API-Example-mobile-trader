...
import 'package:myfxhistory_app/models/order_data.dart';
import 'package:myfxhistory_app/models/order_pos_data_base.dart';
...
import 'package:myfxhistory_app/remote/remote_api_manager.dart';
...
class EditOrderPositionScreen extends StatefulWidget {
  const EditOrderPositionScreen({super.key, this.orderId, this.positionId})
      : assert(orderId != null || positionId != null,
            'To open edit screen at least one of id should be provided');
  ...
  @override
  State<EditOrderPositionScreen> createState() =>
      _EditOrderPositionScreenState();
}
...
class _EditOrderPositionScreenState extends State<EditOrderPositionScreen> {
  ...
  int _margin(int volume) =>
      ((_orderPosData.isBuy ? _marginBuy : _marginSell) * volume).round();
  ...
  bool get _isTPMinusBtnDisabled {
    ...
    final int? rate =
        _isOrder ? _whenRateIs : (data.isBuy ? symbol?.ask : symbol?.bid);

    return rate == null ||
        profitDisatance == null ||
        (data.isBuy &&
            _takeProfit <= (rate + profitDisatance * (data.isBuy ? 1 : -1)));
  }
  ...
  bool get _isSLMinusBtnDisabled {
    ...
    final int? rate =
        _isOrder ? _whenRateIs : (data.isBuy ? symbol?.bid : symbol?.ask);
    ...
    return rate == null ||
        ...
        (!data.isBuy &&
            _stopLoss <=
                (rate +
                    (data.isBuy ? -1 : 1) *
                        (lossDistance + (_isOrder ? symbol?.spread ?? 1 : 1))));
  }
  ...
  bool get _isTSMinusBtnDisabled {
    ...
    final int tsRate = _orderPosData.rate +
        (_orderPosData.isBuy ? -1 : 1) * _trailingStop.value!;
    final int rate = _isOrder
        ? _whenRateIs
        : (_orderPosData.isBuy ? symbol!.ask! : symbol!.bid!);
    final int extremeRate = rate +
        (_orderPosData.isBuy ? -1 : 1) *
            (symbol!.stopLossDistance + symbol.spread);
  ...
  bool get _isActionBtnDisabled {
    if (_isOrder) {
      ...
      return (order.volume == _tradingAmount) &&
          ...
          ((order.takeProfit == null && _takeProfit <= 0) ||
              order.takeProfit == _takeProfit) &&
          ((order.stopLoss == null && _stopLoss <= 0) ||
              order.stopLoss == _stopLoss) &&
          ((!order.trailingStopLoss && !_trailingStop.enabled) ||
              order.trailingStopDistance == _trailingStop.value);
    } else {
      final PositionData position = _orderPosData as PositionData;

      return (position.stopLoss == _stopLoss ||
              (_stopLoss < 0 && position.stopLoss == null)) &&
          (position.takeProfit == _takeProfit ||
              (_takeProfit < 0 && position.takeProfit == null)) &&
          ((!position.trailingStopLoss && !_trailingStop.enabled) ||
              (position.trailingStopDistance == _trailingStop.value));
    }
  }
  ...
  void initState() {
    ...
    if (_isOrder) {
      ...
      if (_order?.expireAt != null)
        _gtdDateTime = _order!.expireAt!.add(Duration.zero);
    ...
    final SymbolData? symbol = data.symbol;
    if (symbol?.id != null) {
      GetIt.I<RemoteAPIManager>()
          .getSpotSubscriptionManager(demo: trader.demo)
          .subscribe(trader.id, <int>[symbol!.id]);

      if (symbol.details == null) {
        GetIt.I<RemoteAPIManager>()
            .getAPI(demo: trader.demo)
            .sendSymbolById(trader.id, <int>[symbol.id]);
      }
    }
  }
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Column(children: <Widget>[
          if (_isOrder)
            for (final Widget w
                in _orderHeader(l10n, orderPosData as OrderData))
              w,
          if (!_isOrder)
            for (final Widget w
                in _positionHeader(l10n, orderPosData as PositionData))
              w,
          ...
          _sectionHeader(
              l10n.takeProfit, _isTPEnabled, false, _toggleTakeProfit),
          if (_isTPEnabled)
            for (final Widget w
                in _takeProfitSection(l10n, orderPosData, symbol))
              w,
          Container(height: 1, color: THEME.dividerLight()),
          _sectionHeader(l10n.stopLoss, _isSLEnabled,
              _orderPosData.garanteedStopLoss, _toggleStopLoss),
          if (_isSLEnabled)
            for (final Widget w in _stopLossSection(l10n, orderPosData, symbol))
              w,
          Container(height: 1, color: THEME.dividerLight()),
          _sectionHeader(l10n.trailingStop, _trailingStop.enabled, false,
              _toggleTrailingStop),
          if (_trailingStop.enabled)
            for (final Widget w in _trailingStopSection(
                l10n, symbol, orderPosData.isBuy, orderPosData.volume))
              w,
          Container(height: 1, color: THEME.dividerLight()),
          Padding(
  ...
  List<Widget> _positionHeader(AppLocalizations l10n, PositionData position) {
    return <Widget>[
      ...
      Row(children: <Widget>[
        ...
        Text(position.isBuy ? l10n.buy : l10n.sell,
            style: THEME.texts.headingRegular),
        const SizedBox(width: 6),
        Text(position.symbol?.name ?? '----',
            style: THEME.texts.headingRegular),
        const Spacer(),
        Text(
          position.formattedPnlWithCurrency,
          style: THEME.texts.headingBold
              .copyWith(color: position.netPnl < 0 ? THEME.red : THEME.green),
        ),
        const SizedBox(width: 16),
      ]),
      const SizedBox(height: 4),
      ...
      _headerLine(
          l10n.openingPrice, position.formattedRate(system: position.rate)),
      _headerLine(l10n.currentRate,
          position.formattedRate(system: position.currentRate)),
  ...
  List<Widget> _orderHeader(AppLocalizations l10n, OrderData order) {
    return <Widget>[
      ...
      Row(children: <Widget>[
        const SizedBox(width: 16),
        Text(order.isBuy ? l10n.buy : l10n.sell,
            style: THEME.texts.headingRegular),
        const SizedBox(width: 6),
        Text(order.symbol?.name ?? '----', style: THEME.texts.headingRegular),
        const Spacer(),
        Text('@${order.formattedRate(system: order.rate)}',
            style: THEME.texts.headingBold),
        const SizedBox(width: 16),
      ]),
      const SizedBox(height: 4),
      ...
      _headerLine(
          l10n.currentRate, order.formattedRate(system: order.currentRate)),
      _headerLine(
          l10n.goodTill,
          order.expireAt == null
              ? l10n.cancelled
              : order.expireAt!.formatted()),
  ...
  Widget _sectionHeader(
      String label, bool isExpanded, bool isDisabled, Function(bool) onChange) {
    return Padding(
      ...
      child: IntrinsicHeight(
        child: Row(children: <Widget>[
          ...
          const Spacer(),
          WrappedSwitch(
              selected: isExpanded, onChange: onChange, disabled: isDisabled),
        ]),
      ),
    );
  }
  ...
  List<Widget> _amountSection(AppLocalizations l10n, SymbolData? symbol) {
    ...
    return <Widget>[
      ...
      Flex(direction: Axis.horizontal, children: <Widget>[
        ...
        Expanded(
            child: Center(
                child: Text(details?.data.measurementUnits ?? 'n/a',
  ...
  List<Widget> _whenRateIsSection(AppLocalizations l10n, SymbolData? symbol) {
    ...
    final double distance =
        rate != null ? (_whenRateIs - rate).abs() / rate * 100 : 0;

    return <Widget>[
      Container(height: 1, color: THEME.dividerLight()),
      _sectionHeader(
          l10n.buySellWhenRateIs(_orderPosData.isBuy ? l10n.buy : l10n.sell),
          true,
          false,
          (_) => null),
      ...
      Row(children: <Widget>[
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Text(_gtdDateTime!.formatted('d MMMM y'),
  ...
  List<Widget> _takeProfitSection(
      AppLocalizations l10n, OrderPosDataBase data, SymbolData? symbol) {
    String expProfitStr = '---';

    if (symbol != null) {
      final int rate = _isOrder ? _whenRateIs : data.rate;
      final int tpRateDist =
          data.isBuy ? _takeProfit - rate : rate - _takeProfit;
      final double expProfit =
          SymbolData.humanicRateFromSystem(tpRateDist) * data.humanicVolume;
  ...
  List<Widget> _stopLossSection(
      AppLocalizations l10n, OrderPosDataBase data, SymbolData? symbol) {
    String expLossStr = '---';
    if (symbol != null) {
      final int rate = _isOrder ? _whenRateIs : data.rate;
      final int slRateDist = data.isBuy ? rate - _stopLoss : _stopLoss - rate;
      final double expLoss =
          SymbolData.humanicRateFromSystem(slRateDist) * data.humanicVolume;
      final AssetData? asset = symbol.trader.tree.asset(symbol.quoteAssetId);
      expLossStr =
  ...
  List<Widget> _trailingStopSection(
      AppLocalizations l10n, SymbolData? symbol, bool isBuy, int volume) {
    ...
    final bool isTSCurrent =
        _orderPosData.trailingStopDistance == _trailingStop.value ||
            (_trailingStop.value == null);
    final int? rate;

    if (isTSCurrent) {
      rate = _orderPosData.stopLoss;
    } else {
      rate = (_isOrder ? _whenRateIs : _orderPosData.rate) +
          (isBuy ? -1 : 1) * (_trailingStop.value ?? 0);
    }
    ...
    return <Widget>[
      ...
      Flex(
        ...
        children: <Widget>[
          const Spacer(),
          BuySellIncrementField(
            value: _trailingStop.value == null
                ? null
                : _trailingStop.value! / pow(10, 5 - pipPos),
  ...
  void _onChangeTradingAmount(double value) {
    ...
    if (_margin(amount) > GetIt.I<UserState>().selectedTrader.freeMargin)
      return;
  ...
  void _correctLimitRate() {
    if (_whenRateIs > 0) {
      ...
      _whenRateIs = symbol.details!
          .cutOffExtraDigitsFromRate(_whenRateIs, _orderPosData.isBuy);

      final int? rate = _orderPosData.currentRate;
      if (rate == null) return;

      if (_whenRateIs == rate) {
        _whenRateIs += (_orderPosData.isBuy ? 1 : -1) *
  ...
  void _correctTakeProfit() {
    ...
    _takeProfit = symbol!.details!
        .cutOffExtraDigitsFromRate(_takeProfit, !_orderPosData.isBuy);

    final int extremeRate = rate + takeProfitDisatance * (data.isBuy ? 1 : -1);
    if ((data.isBuy && _takeProfit < extremeRate) ||
        (!data.isBuy && _takeProfit > extremeRate)) {
  ...
  void _correctStopLoss() {
    ...
    final int extremeRate = rate +
        (lossDistance + (_isOrder ? symbol.spread : 0)) * (data.isBuy ? -1 : 1);
    if ((data.isBuy && _stopLoss > extremeRate) ||
        (!data.isBuy && _stopLoss < extremeRate)) {
  ...
  void _onChangeTrailingStop(double value) {
    ...
    if (symbol != null && symbol.details != null) {
      if (symbol.details!.data.pipPosition > 1)
  ...
  void _correctTrailingStop() {
    ...
    final int tsRate = _orderPosData.rate +
        _trailingStop.value! * (_orderPosData.isBuy ? -1 : 1);
    final int rate = _isOrder
        ? _whenRateIs
        : (_orderPosData.isBuy ? symbol!.ask! : symbol!.bid!);
    final int extremeRate = rate +
  ...
  Future<void> _updateExpactedMargin() async {
    ...
    if (symbol.details == null)
      await remoteApi.sendSymbolById(symbol.trader.id, <int>[symbol.id]);
  ...
  Future<void> _onTapActionButton() async {
    ...
    if (_isOrder) {
      ...
      final bool isLimit = order.isBuy
          ? order.currentRate! > _whenRateIs
          : order.currentRate! < _whenRateIs;
      ...
      if (order.isLimit == isLimit) {
        ...
        if (_trailingStop.enabled &&
          ...
          stopLoss =
              _whenRateIs + (order.isBuy ? -1 : 1) * _trailingStop.value!;
        ...
        if (!GetIt.I<UserState>().dontShowReversOrderPopup) {
          final PopupResult result = await GetIt.I<PopupManager>()
              .askToCancelOrderBeforeNewOne(
                  AppLocalizations.of(context)!, order.id);
          if (!result.agree) {
            GetIt.I<AppState>().setUIBlocked(false);
            return;
          } else if (result.payload[Popup.PAYLOAD_CHECKBOX_KEY] == true)
            GetIt.I<UserState>().setDontShowReversOrderPopup();
        }
        ...
        if (simultaneousState.isOrderPaired(order.trader.id, order.id)) {
          final Map<int, int> pairedOrders =
              simultaneousState.getPairedOrders(order.trader.id, order.id);

          final PopupResult result = await GetIt.I<PopupManager>()
              .askToApplySimultaneousChanges(
                  AppLocalizations.of(context)!, false, pairedOrders.keys);
          if (result.agree && result.payload['all'] != true) {
            simultaneousState.removeOrderFromPair(order.trader.id, order.id);
            final int? newId =
                await _replaceOrderWithNew(order, order.symbol?.details);
            ...
            for (final int accId in pairedOrders.keys.toList()) {
              try {
                ...
                final OrderData order =
                    trader.ordersManager.activityBy(id: pairedOrders[accId])!;
                final int? newId =
                    await _replaceOrderWithNew(order, symbolDetails);
                if (newId != null) {
  ...
  Future<int?> _replaceOrderWithNew(
      OrderData oldOrder, SymbolDetailsData? symbolDetails) async {
    ...
    if (closed) {
      ...
      try {
        final ProtoMessage resp = await remoteAPI.sendNewOrderForOrder(
          ...
          takeProfit:
              _isTPEnabled ? (_takeProfit - oldOrder.currentRate!).abs() : null,
          trailingStop: _trailingStop.enabled,
          stopLoss: _trailingStop.enabled
              ? _trailingStop.value
              : (_isSLEnabled ? (_whenRateIs - _stopLoss).abs() : null),
          guaranteedStopLoss: trader.isLimitedRisk,
        );

        if (resp is ProtoOAExecutionEvent &&
            resp.executionType == ProtoOAExecutionType.orderAccepted) {
        ...
        GetIt.I<PopupManager>().showError(
            l10n,
            l10n.getServerErrorDescription(e.errorCode) ??
                e.description ??
                e.errorCode);
        return null;
      } catch (e) {
