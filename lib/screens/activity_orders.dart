...
class _ActivityOrdersState extends State<ActivityOrders> {
  ...
  Widget build(BuildContext context) {
    ...
    if (orders.isEmpty) {
      return Container(
        ...
        child: Text(l10n.noActiveOrders,
  ...
  Widget _listItem(AppLocalizations l10n, OrderData order) {
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        color: _expandedOrders.contains(order.id)
          ...
          GestureDetector(
            ...
            child: Row(children: <Widget>[
              ...
              Container(
                ...
                child: Text(order.isBuy ? l10n.buy : l10n.sell,
  ...
  List<Widget> _itemDetails(AppLocalizations l10n, OrderData order) {
    ...
    if (order.trailingStopLoss) {
      ...
      if (trailingStopDist != null) {
        trailingStop =
            order.symbol?.details?.formattedPips(system: trailingStopDist) ??
    ...
    return <Widget>[
      ...
      if (order.executedVolume > 0)
        Row(children: <Widget>[
          ...
          const Spacer(),
          Text(
            order.symbol?.details
                    ?.formattedVolumeWithUnits(system: order.executedVolume) ??
                SymbolData.formattedVolumeDefault(system: order.executedVolume),
      ...
      Row(children: <Widget>[
        ...
        const Spacer(),
        Text(order.formattedVolumeWithUnits,
            style: THEME.texts.bodyMediumSecondary),
        const SizedBox(width: 16),
      ]),
      Row(children: <Widget>[
      ...
      Row(children: <Widget>[
        ...
        Text(
          order.takeProfit == null
              ? l10n.no
              : order.formattedRate(system: order.takeProfit),
          style: THEME.texts.bodyMediumSecondary,
        ),
        const SizedBox(width: 16),
      ]),
      if (!order.trailingStopLoss)
        Row(children: <Widget>[
          ...
          Text(
            order.stopLoss == null
                ? l10n.no
                : order.formattedRate(system: order.stopLoss),
            style: THEME.texts.bodyMediumSecondary,
          ),
          const SizedBox(width: 16),
        ]),
  ...
  void _onTapEditOrder(OrderData order) {
    ...
    } else if (order.symbol?.details?.data.tradingMode ==
