...
import 'package:myfxhistory_app/extensions.dart';
import 'package:myfxhistory_app/l10n/localization_helper.dart';
import 'package:myfxhistory_app/l10n/localizations.dart';
import 'package:myfxhistory_app/logger.dart';
import 'package:myfxhistory_app/main.dart';
import 'package:myfxhistory_app/models/asset_data.dart';
import 'package:myfxhistory_app/models/internal_application_error.dart';
import 'package:myfxhistory_app/models/position_data.dart';
import 'package:myfxhistory_app/models/symbol_data.dart';
import 'package:myfxhistory_app/models/trader_data.dart';
import 'package:myfxhistory_app/popups/popup.dart';
import 'package:myfxhistory_app/popups/popup_manager.dart';
import 'package:myfxhistory_app/remote/proto.dart';
import 'package:myfxhistory_app/remote/remote_api.dart';
import 'package:myfxhistory_app/states/app_state.dart';
import 'package:myfxhistory_app/states/simultaneous_trading_state.dart';
import 'package:myfxhistory_app/states/user_state.dart';
import 'package:myfxhistory_app/widgets/button_primary.dart';
import 'package:myfxhistory_app/widgets/buy_sell_increment_field.dart';
import 'package:myfxhistory_app/widgets/side_menu.dart';
import 'package:myfxhistory_app/widgets/symbol_buy_sell.dart';
import 'package:myfxhistory_app/widgets/wrapped_appbar.dart';
import 'package:myfxhistory_app/widgets/wrapped_switch.dart';
...
class _BuySellScreenState extends State<BuySellScreen> {
  ...
  void initState() {
    ...
    final SymbolData? symbol =
        userState.selectedTrader.tree.symbol(widget.symbolId);
    ...
    symbol.getDetailsData().then((_) {
      ...
      userState
          .resotreSymbolVolume(
              traderId: userState.selectedTraderId, symbolId: symbol.id)
          .then((int? value) =>
              setState(() => _tradingAmount = value ?? _tradingAmount));

      if (userState.selectedTrader.isLimitedRisk) _onToggleSL(true);

      _expMarginReqTimer = Timer.periodic(
        const Duration(seconds: 2),
        (Timer timer) => _updateExpactedMargin()
  ...
  Widget build(BuildContext context) {
    ...
    final SymbolData? symbol =
        context.watch<UserState>().selectedTrader.tree.symbol(widget.symbolId);

    return Scaffold(
      appBar: WrappedAppBar(
        title:
            "${_isBuy ? l10n.buy : l10n.sell} ${symbol != null ? symbol.name.toUpperCase() : ""}",
        showBack: true,
      ),
      ...
      body: SingleChildScrollView(
        child: Column(children: <Widget>[
          ...
          _divider(),
          _sectionHeader(l10n.buySellWhenRateIs(_isBuy ? l10n.buy : l10n.sell),
              _whenRateIs > 0, false, _onToggleWhenRateIs),
          if (_whenRateIs > 0)
          ...
          _divider(),
          _sectionHeader(l10n.stopLoss, _isSLEnabled,
              symbol?.trader.isLimitedRisk == true, _onToggleSL),
          if (_isSLEnabled)
            for (final Widget item in _stopLossSection(l10n, symbol)) item,
          _divider(),
          _sectionHeader(
              l10n.trailingStop, _trailingStop > 0, false, _onToggleTS),
          if (_trailingStop > 0)
          ...
          Padding(
            ...
            child: ButtonPrimary(
                label: _isBuy ? l10n.buy : l10n.sell,
  ...
  List<Widget> _amountSection(AppLocalizations l10n, SymbolData? symbol) {
    ...
    if (details != null) {
      decimals = details.data.minVolume < 10 || details.data.stepVolume < 10
          ? 2
          : (details.data.minVolume < 100 || details.data.stepVolume < 100
              ? 1
              : 0);
    }
    ...
    if (margin > 0) {
      expectedMargin =
          symbol?.trader.formattedMoneyWithCurrency(cents: margin) ??
              expectedMargin;
    } else if (symbol?.trader.isLimitedRisk == true) {
      expectedMargin = symbol!.trader.formattedMoneyWithCurrency(
          money: _expMarginConvertRate * _expextedLoss(symbol));
    }
    ...
    return <Widget>[
      ...
      Flex(
        ...
        children: <Widget>[
          ...
          BuySellIncrementField(
            ...
            minusButtonDisabled:
                (details?.data.minVolume ?? 0) >= _tradingAmount,
          ...
          Expanded(
            child: Container(
              ...
              child: Text(details?.data.measurementUnits ?? '',
      ...
      Text('${l10n.expectedMargin}: $expectedMargin',
          style: THEME.texts.bodyRegular),
      const SizedBox(height: 16),
    ];
  }

  Widget _sectionHeader(String label, bool isExpanded, bool disabledSwitch,
    ...
    return Padding(
      ...
      child: IntrinsicHeight(
        ...
        const Spacer(),
        WrappedSwitch(
            selected: isExpanded, disabled: disabledSwitch, onChange: onChange),
      ])),
    );
  }
    ...
    return <Widget>[
      Stack(children: <Widget>[
        Column(children: <Widget>[
          ...
          Text('${l10n.distance}: ${distance.toComaSeparated(decimals: 2)}%',
              style: THEME.texts.bodyMedium),
          const SizedBox(height: 8),
          if (_dateTimePicker)
          ...
          child: GestureDetector(
            ...
            child: Container(
              ...
              child: SvgPicture.asset(
                ...
                width: 32,
                height: 32,
                colorFilter: (_dateTimePicker
                        ? THEME.buySellTimerIconSelected()
                        : THEME.buySellTimerIcon())
                    .asFilter,
              ),
            ),
          ),
      ...
      Row(children: <Widget>[
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Text(_gtdDateTime!.formatted('HH:mm'),
                style: THEME.texts.inputIncremenet),
          ),
        ),
        const SizedBox(width: 12),
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Text(_gtdDateTime!.formatted('d MMM y'),
                style: THEME.texts.inputIncremenet, softWrap: false),
          ),
        ),
        const SizedBox(width: 16),
      ]),
  ...
  List<Widget> _takeProfitSection(AppLocalizations l10n, SymbolData? symbol) {
    ...
    if (symbol != null) {
      final int? rate =
          _whenRateIs < 0 ? (_isBuy ? symbol.ask : symbol.bid) : _whenRateIs;
      final int tpRateDist = ((rate ?? _takeProfit) - _takeProfit).abs();
      final double expProfit =
          SymbolData.humanicRateFromSystem(tpRateDist) * _tradingAmount / 100;
      final AssetData? asset = symbol.trader.tree.asset(symbol.quoteAssetId);
      expProfitStr =
          asset?.formattedValue(expProfit, units: true) ?? expProfit.toString();
    }
    ...
    return <Widget>[
      ...
      const SizedBox(height: 6),
      Text('${l10n.expectedProfit}: $expProfitStr',
          style: THEME.texts.bodyRegular),
      const SizedBox(height: 8),
    ];
  }
  ...
  List<Widget> _stopLossSection(AppLocalizations l10n, SymbolData? symbol) {
    ...
    if (symbol != null) {
      ...
      final AssetData? asset = symbol.trader.tree.asset(symbol.quoteAssetId);
      expLossStr =
          asset?.formattedValue(expLoss, units: true) ?? expLoss.toString();
    }
      ...
      Flex(
        ...
        children: <Widget>[
          ...
          Expanded(
              child: Container(
                  alignment: Alignment.center,
                  child: Text(l10n.pips, style: THEME.texts.bodyMedium))),
        ],
      ),
      const SizedBox(height: 6),
      Text(
  ...
  Widget _incrementTimeButton(
      String iconName, bool isEnabled, Function() onTap) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: SvgPicture.asset('assets/svg/$iconName.svg',
            width: 8, colorFilter: THEME.onBackground().asFilter),
      ),
    );
  }
  ...
  DateTime get _defaultGTDValue {
    return DateTime.now()
        ...
        .updateTime(seconds: 0, milliseconds: 0, microseconds: 0);
  }

  int _margin(int volume) =>
      ((_isBuy ? _marginBuy : _marginSell) * volume).round();
  ...
  double _expextedLoss(SymbolData symbol) {
    if (_trailingStop.isNegative) {
      ...
      return SymbolData.humanicRateFromSystem(slRateDist) *
          _tradingAmount /
          100;
    } else {
      return SymbolData.humanicRateFromSystem(_trailingStop) *
          _tradingAmount /
          100;
    }
  }
  ...
  bool _isAmountPlusBtnDisabled(SymbolDetailsData? details) {
    return details == null ||
        details.data.maxVolume <= _tradingAmount ||
        _margin(_tradingAmount + details.data.stepVolume) >
            details.symbol.trader.freeMargin;
  }
  ...
  bool _isTPMinusBtnDisabled(SymbolData? symbol) {
    ...
    final int rate =
        _whenRateIs < 0 ? (_isBuy ? symbol.ask : symbol.bid) ?? 0 : _whenRateIs;
    final int correctionRate =
        (rate + direction * symbol.takeProfitDisatance).toInt();

    return (_isBuy && _takeProfit <= correctionRate) ||
        (!_isBuy && _takeProfit <= (symbol.pipSize ?? 0));
  }
  ...
  bool _isSLMinusBtnDisabled(SymbolData? symbol) {
    ...
    final int direction = _isBuy ? -1 : 1;
    final int rate =
        _whenRateIs < 0 ? (_isBuy ? symbol.bid : symbol.ask) ?? 0 : _whenRateIs;
    final int distance =
        symbol.stopLossDistance + (_whenRateIs > 0 ? symbol.spread : 0);
    final int correctionRate = (rate + direction * distance).toInt();

    return (_isBuy && _stopLoss <= (symbol.pipSize ?? 0)) ||
        (!_isBuy && _stopLoss <= correctionRate);
  }
  ...
  bool _isTSMinusBtnDisabled(SymbolData? symbol) {
    ...
    return _trailingStop <=
        max(symbol.stopLossDistance, 5 * (symbol.pipSize ?? 1)) + symbol.spread;
  }
  ...
  Future<void> _updateExpactedMargin() async {
    ...
    if (!trader.isLimitedRisk ||
        trader.limitedRiskMarginCalculationStrategy ==
            ProtoOALimitedRiskMarginCalculationStrategy.accordingToLeverage) {
      final RemoteApi remoteApi = trader.remoteApi;
      final ProtoOAExpectedMarginRes marginResp =
          await remoteApi.sendExpectedMargin(
        symbol.trader.id,
        symbol.id,
      ...
      _expMarginConvertRate = await symbol.quoteAsset
              ?.getConversionRateToAsset(trader.currencyId) ??
          1;

      if (trader.limitedRiskMarginCalculationStrategy ==
          ProtoOALimitedRiskMarginCalculationStrategy.accordingToGsl) {
        _marginBuy = _marginSell = -1;
      } else {
        final ProtoOAExpectedMarginRes marginResp =
            await trader.remoteApi.sendExpectedMargin(
          symbol.trader.id,
          symbol.id,
          <int>[symbol.details!.data.minVolume],
        );
        ...
        final double serverCalcs =
            trader.currencyAsset!.humanicValue(_margin(_tradingAmount));
  ...
  void _onAmountChanged(double value) {
    ...
    final SymbolData? symbol =
        GetIt.I<UserState>().selectedTrader.tree.symbol(widget.symbolId);
  ...
  void _onToggleWhenRateIs(bool enable) {
    ...
    if (enable &&
        ((_isBuy && symbol.ask != null) || (!_isBuy && symbol.bid != null))) {
      _whenRateIs =
          (_isBuy ? symbol.ask! * 1.001 : symbol.bid! * 0.999).round();
  ...
  Future<void> _selectGTDTime() async {
    ...
    final TimeOfDay? time = await showTimePicker(
        context: context, initialTime: TimeOfDay.fromDateTime(_gtdDateTime!));
    if (time != null) {
      _gtdDateTime =
          _gtdDateTime!.updateTime(hours: time.hour, minutes: time.minute);
      _correctGTDDate();
      setState(() {});
    }
  }
  ...
  Future<void> _selectGTDDate() async {
    ...
    if (newDate != null) {
      _correctGTDDate();
      setState(() {
        _gtdDateTime = newDate.updateTime(
            hours: _gtdDateTime?.hour ?? 0, minutes: _gtdDateTime?.minute ?? 0);
      });
    }
  }
  ...
  void _onToggleTP(bool enable) {
    if (enable) {
      ...
      if (cached != null) {
        ...
        final int direction = _isBuy ? 1 : -1;
        final int rate = _whenRateIs < 0
            ? (_isBuy ? symbol.ask : symbol.bid) ?? 0
            : _whenRateIs;
        _takeProfit =
            (rate + direction * symbol.takeProfitDisatance * 2).toInt();
      }
    } else {
      _takeProfit = -1;
    }
  ...
  void _onToggleSL(bool enable) {
    if (enable) {
      ...
      if (cached != null) {
        ...
        final int direction = _isBuy ? -1 : 1;
        final int rate = _whenRateIs < 0
            ? (_isBuy ? symbol.bid : symbol.ask) ?? 0
            : _whenRateIs;
  ...
  void _onChangeTS(double ts) {
    ...
    if ((symbol?.details?.data.pipPosition ?? 0) > 1)
      ts = (ts * 10).round() / 10;
  ...
  void _correctTradingAmount() {
    final UserState userState = GetIt.I<UserState>();
    final ProtoOASymbol? details =
        userState.selectedTrader.tree.symbol(widget.symbolId)?.details?.data;
    ...
    if (_margin(_tradingAmount) > freeMargin) {
      if (details.minVolume < _tradingAmount) {
        int step = 0;
        while (
            _margin(_tradingAmount - step * details.stepVolume) > freeMargin) {
          step++;
        }
        _tradingAmount =
            max(_tradingAmount - step * details.stepVolume, details.minVolume);
      }
    }
  }
  ...
  void _correctLimitRate() {
    if (_whenRateIs > 0) {
      ...
      _whenRateIs = symbol.details!
          .cutOffExtraDigitsFromRate(_whenRateIs, _whenRateIs < (rate ?? 0));

      if (rate == null) return;

      if (symbol.trader.isLimitedRisk &&
          _whenRateIs <=
              symbol.stopLossDistance + symbol.spread + symbol.pipSize!) {
        _whenRateIs = symbol.stopLossDistance + symbol.spread + symbol.pipSize!;
      } else if (_whenRateIs == rate) {
        _whenRateIs += (_isBuy ? 1 : -1) *
            pow(10, 5 - symbol.details!.data.digits).toInt();
      }
    }
  }
  ...
  void _correctTPRate() {
    ...
    _takeProfit = max(
        symbol.details!.cutOffExtraDigitsFromRate(_takeProfit, !_isBuy),
        symbol.pipSize!);
  ...
  void _correcntSLRate() {
    ...
    _stopLoss = max(
        symbol.details!.cutOffExtraDigitsFromRate(_stopLoss, _isBuy),
        symbol.pipSize!);
  ...
  void _correctTSValue() {
    ...
    _trailingStop =
        max(max(distance, 5 * (symbol.pipSize ?? 10)), _trailingStop);
  ...
  void _onTapActionButton() {
    final SimultaneousTrdaingState simultaneousState =
        GetIt.I<SimultaneousTrdaingState>();
    final UserState userState = GetIt.I<UserState>();
    final TraderData trader = userState.selectedTrader;
    if (trader.accessRights.index >= ProtoOAAccessRights.closeOnly.index) {
      GetIt.I<PopupManager>()
          .showTraderSelection(AppLocalizations.of(context)!, trader, false);
      return;
    }
    ...
    if (symbol == null) {
      ...
      GetIt.I<PopupManager>()
          .showSomeErrorOccurred(AppLocalizations.of(context)!);
      return;
    }

    if (simultaneousState.isAccountPaired(userState.selectedTraderId)) {
      _openActivityForSimultaneousTrading(
          userState, simultaneousState, trader, symbol);
    } else {
  ...
  Future<void> _openActivityForSimultaneousTrading(
    ...
    final PopupResult result =
        await GetIt.I<PopupManager>().askToOpenSimultaneousPosition(
    ...
    if (result.payload['all'] != true)
      return _openActivityForSingleAccount(userState, symbol);

    final Map<int, Iterable<SymbolData>> pairedSymbols =
        <int, Iterable<SymbolData>>{};
    ...
    for (final int traderId in simultaneousState.pariedAccounts) {
      ...
      pairedSymbols[traderId] = pairedTrader?.tree
              .findSymbolForSimultaneousTrading(name: symbol.name) ??
          <SymbolData>[];
    }

    final List<int> notPairedAccounts = <int>[];
    for (final int traderId in pairedSymbols.keys) {
      if (pairedSymbols[traderId] == null || pairedSymbols[traderId]!.isEmpty)
        notPairedAccounts.add(traderId);
    }

    if (notPairedAccounts.isNotEmpty) {
      GetIt.I<PopupManager>().showSimultaneousSymbolPairNotFound(
          AppLocalizations.of(context)!, symbol.name, notPairedAccounts);
      pairedSymbols.removeWhere((int key, Iterable<SymbolData> value) =>
          notPairedAccounts.contains(key));
    }

    GetIt.I<AppState>().setUIBlocked(true);
    final bool isLimit =
        _isBuy ? symbol.ask! > _whenRateIs : symbol.bid! < _whenRateIs;
    ...
    for (final int traderId in pairedSymbols.keys) {
      ...
      for (final SymbolData symbol in symbols) {
        try {
          pairedActivities[trader.id] = await (_whenRateIs < 0
              ? _openPosition(trader, symbol, symbolDetails, tradingAmount,
                  ask: ask, bid: bid, noErrorPopup: true)
              : _openOrder(trader, symbol, symbolDetails, isLimit,
                  noErrorPopup: true));
          success = true;
          break;
        } catch (err) {
          Logger.error(
              'Error occurred at opening new activity: ${err.runtimeType}("$err")');
          if (err is InternalApplicationError &&
              err.code ==
                  InternalApplicationErrorCodes.SYMBOL_NOT_INITIALIZED) {
            GetIt.I<PopupManager>()
                .showSomeErrorOccurred(AppLocalizations.of(context)!);
          }
        }
      ...
      if (!success)
        GetIt.I<PopupManager>().showSimultaneousExecutionFaildPopup(
            AppLocalizations.of(context)!, trader.name, symbol.name);
    }
  ...
  Future<void> _openActivityForSingleAccount(
      UserState userState, SymbolData symbol) async {
    GetIt.I<AppState>().setUIBlocked(true);

    try {
      if (_whenRateIs < 0) {
        await _openPosition(
            userState.selectedTrader, symbol, symbol.details, _tradingAmount);
      } else {
        await _openOrder(userState.selectedTrader, symbol, symbol.details,
            _isBuy ? symbol.ask! > _whenRateIs : symbol.bid! < _whenRateIs);
      }
      ...
      Logger.error(
          'Error occurred at opening new activity: ${err.runtimeType}("$err")');
      if (err is InternalApplicationError &&
          err.code == InternalApplicationErrorCodes.SYMBOL_NOT_INITIALIZED) {
        GetIt.I<PopupManager>()
            .showSomeErrorOccurred(AppLocalizations.of(context)!);
      }
    }
  ...
  Future<int> _openOrder(TraderData trader, SymbolData symbol,
      SymbolDetailsData? symbolDetails, bool isLimit,
      {bool noErrorPopup = false}) async {
    final RemoteApi remoteAPI = trader.remoteApi;
    ...
    try {
      final ProtoOAExecutionEvent resp = await _handleRequestAndValidate(
        ...
        remoteAPI.sendNewOrderForOrder(
          ...
          stopLoss: _isTSEnabled
              ? _trailingStop
              : (_isSLEnabled ? (_stopLoss - _whenRateIs).abs() : null),
      ...
      final UserState userState = GetIt.I<UserState>();
      userState.saveSymbolVolume(
          traderId: userState.selectedTraderId,
          symbolId: symbol.id,
          volume: _tradingAmount);

      if (_isTSEnabled)
        GetIt.I<UserState>()
            .trailingStopValues
            .updateForOrder(trader.id, resp.order!.orderId, _trailingStop);

      GetIt.I<PopupManager>().showPendingOrderWasCreated(
          AppLocalizations.of(context)!, resp,
      ...
      Logger.error(
          'Error occurred for new order for trader(${trader.id}) with symbol(${symbol.id}): (${err.runtimeType}) $err');
      GetIt.I<PopupManager>()
          .showSomeErrorOccurred(AppLocalizations.of(context)!);
  ...
  Future<int> _openPosition(
    ...
    throwIf(
        ...
        InternalApplicationError(
            InternalApplicationErrorCodes.SYMBOL_NOT_INITIALIZED,
            'Symbol rate is not initialized'));
    ...
    try {
      ProtoOAExecutionEvent resp = await _handleRequestAndValidate(
        ...
        remoteAPI.sendNewOrderForPosition(
          ...
          stopLoss: _isTSEnabled
              ? _trailingStop
              : (_isSLEnabled ? (_stopLoss - rate!).abs() : null),
          guaranteedStopLoss: trader.isLimitedRisk,
        ),
        noError: noErrorPopup,
      );
      ...
      final PositionData? position =
          trader.positionsManager.activityBy(id: resp.position?.positionId);
      final bool posHadSLTP =
      ...
      userState.saveSymbolVolume(
          traderId: userState.selectedTraderId,
          symbolId: symbol.id,
          volume: tradingAmount);

      // waiting for order with execution type orderFilled
      resp = await _handleRequestAndValidate(
          trader, symbol, remoteAPI.waitForResponse(resp.cmdId!),
          noError: noErrorPopup);
      ...
      if (resp.position!.tradeData.volume == 0) {
        ...
        userState.trailingStopValues
            .clearFromPosition(trader.id, resp.position!.positionId);
      } else {
        // update cached trailing stop values
        if (_isTSEnabled) {
          userState.trailingStopValues.updateForPosition(
              trader.id, resp.position!.positionId, _trailingStop);
        } else {
          userState.trailingStopValues
              .clearFromPosition(trader.id, resp.position!.positionId);
        }

        final bool isPostionDecrease = resp.position!.tradeData.tradeSide !=
            resp.order!.tradeData.tradeSide;
        final bool isPositionReversed = !isPostionDecrease &&
            resp.position!.tradeData.volume < resp.order!.executedVolume!;
        final bool isSLTPEndabled =
            _isTPEnabled || _isSLEnabled || _isTSEnabled;
        bool sltpError = false;

        if (posHadSLTP && isPositionReversed) {
          // position is reversed, need to wait for cancel sltp execution event
          try {
            final ProtoMessage cancelSLTP =
                await remoteAPI.waitForResponse(resp.cmdId!);
            if (cancelSLTP is! ProtoOAExecutionEvent ||
                cancelSLTP.order!.orderType !=
                    ProtoOAOrderType.stopLossTakeProfit ||
                cancelSLTP.executionType !=
                    ProtoOAExecutionType.orderCancelled) {
              _validateOpenActivityResponseForSuccess(resp, trader, symbol,
                  noError: noErrorPopup);
              sltpError = true;
            } else {
            ...
            Logger.error(
                'Some error occurred when reversing an position and waited for sltp cancel event of previous order',
                err);
            sltpError = true;
          }
        }
        ...
        } else if ((isPositionReversed && isSLTPEndabled) ||
            (!isPositionReversed && (posHadSLTP || isSLTPEndabled))) {
          // wait for sltp replaced || accepted
          try {
            final ProtoMessage sltpResp =
                await remoteAPI.waitForResponse(resp.cmdId!);
            if (sltpResp is! ProtoOAExecutionEvent ||
                sltpResp.order!.orderType !=
                    ProtoOAOrderType.stopLossTakeProfit ||
                (sltpResp.executionType != ProtoOAExecutionType.orderAccepted &&
                    sltpResp.executionType !=
            ...
            Logger.error(
                'Error occurred for sltpOrder of position ${err.runtimeType}($err)');
            sltpError = true;
          }
        }

        GetIt.I<PopupManager>().handleNewPositionOpenedEvent(
            AppLocalizations.of(context)!, resp,
            symbolDetails: symbolDetails, sltpError: sltpError);
      }
      ...
      Logger.error(
          'Error occurred for new position for trader(${trader.id}) with symbol(${symbol.id}): (${err.runtimeType}) $err');
      GetIt.I<PopupManager>()
          .showSomeErrorOccurred(AppLocalizations.of(context)!);
  ...
  Future<ProtoOAExecutionEvent> _handleRequestAndValidate(
      TraderData trader, SymbolData symbol, Future<ProtoMessage> future,
      {required bool noError}) async {
    try {
      ...
      Logger.log(() =>
          'new order eroor for trader(${trader.id}) with symbol(${symbol.id}): ${err.errorCode}: ${err.description}');
      if (!noError) {
        final AppLocalizations l10n = AppLocalizations.of(context)!;
        GetIt.I<PopupManager>().showError(
            l10n,
            l10n.getServerErrorDescription(err.errorCode) ??
                err.description ??
                err.errorCode);
      }
      ...
      Logger.error(
          'Exception executed for new order for trader(${trader.id}) with symbol(${symbol.id}): (${exc.runtimeType}) $exc');
      if (!noError)
        GetIt.I<PopupManager>()
            .showSomeErrorOccurred(AppLocalizations.of(context)!);
      throw InternalApplicationError(
  ...
  bool _validateOpenActivityResponseForSuccess(
      ProtoMessage resp, TraderData trader, SymbolData symbol,
      {required bool noError}) {
    final String type = _whenRateIs < 0 ? 'position' : 'order';

    if (resp is ProtoOAOrderErrorEvent) {
      Logger.log(() =>
          'Server error for new $type for trader(${trader.id}) with symbol(${symbol.id}): ${resp.errorCode}#${resp.description}');
      if (!noError)
        GetIt.I<PopupManager>()
            .showNewOrderError(AppLocalizations.of(context)!, resp);
      throw InternalApplicationError(
        InternalApplicationErrorCodes.SERVER_ERROR,
      ...
      if (resp.errorCode != null) {
        Logger.log(() =>
            'Server error ${resp.errorCode} for new $type for trader(${trader.id}) with symbol (${symbol.id})');
        if (!noError) {
          final AppLocalizations l10n = AppLocalizations.of(context)!;
          GetIt.I<PopupManager>().showError(
              l10n,
              l10n.getServerErrorDescription(resp.errorCode!) ??
                  resp.errorCode!);
        }
        throw InternalApplicationError(
          InternalApplicationErrorCodes.SERVER_ERROR,
      ...
      } else if (resp.executionType == ProtoOAExecutionType.orderRejected ||
          resp.executionType == ProtoOAExecutionType.orderCancelled) {
        Logger.log(() =>
            'Server rejected or canceled new $type for trader(${trader.id}) with symbol(${symbol.id})');
        if (!noError)
          GetIt.I<PopupManager>().showOrderExecutionFaild(
              AppLocalizations.of(context)!, resp.order!.orderId);
        throw InternalApplicationError(
        ...
        Logger.error(
            'Unhandeled state of response for new $type for trader(${trader.id}) with symbol(${symbol.id})');
        if (!noError)
          GetIt.I<PopupManager>()
              .showSomeErrorOccurred(AppLocalizations.of(context)!);
        throw InternalApplicationError(
      ...
      Logger.error(
          'Unhandled response type(${resp.runtimeType}) for new $type for trader(${trader.id}) with symbol(${symbol.id})');
      if (!noError)
        GetIt.I<PopupManager>()
            .showSomeErrorOccurred(AppLocalizations.of(context)!);
      throw InternalApplicationError(
