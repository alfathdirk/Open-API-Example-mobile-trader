...
class ActivityClosedPositions extends StatefulWidget {
  ...
  @override
  State<ActivityClosedPositions> createState() =>
      _ActivityClosedPositionsState();
}
...
class _ActivityClosedPositionsState extends State<ActivityClosedPositions> {
  ...
  final Map<int, List<ProtoOADealOffset>> _dealOffsets =
      <int, List<ProtoOADealOffset>>{};
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        final int pnl = deal.closePositionDetail!.grossProfit +
              ...
              GestureDetector(
                ...
                child: Row(children: <Widget>[
                  ...
                  Container(
                    ...
                    child: Text(
                      deal.tradeSide == ProtoOATradeSide.buy
                          ? l10n.sell
                          : l10n.buy,
  ...
  List<Widget> _dealDetails(
      AppLocalizations l10n, ProtoOADeal deal, SymbolData? symbol) {
    ...
    if (offsets != null && offsets.isNotEmpty) {
      if (offsets.length == 1) {
        openPrice = symbol?.details
                ?.formattedRate(humanic: offsets.first.executionPrice) ??
            SymbolData.formattedRateDefault(
                humanic: offsets.first.executionPrice);
      } else {
        ...
        for (final ProtoOADealOffset offset in offsets) {
          rateAvarage +=
              offset.volume * (offset.executionPrice ?? 0) / volumeSum;
        }
        openPrice = symbol?.details?.formattedRate(humanic: rateAvarage) ??
            SymbolData.formattedRateDefault(humanic: rateAvarage);
      }

      final int openTS =
          offsets.fold<int>(-1, (int previousValue, ProtoOADealOffset offset) {
    ...
    return <Widget>[
      ...
      Row(children: <Widget>[
        ...
        Text(
          symbol?.details
                  ?.formattedVolumeWithUnits(system: deal.filledVolume) ??
      ...
      Row(children: <Widget>[
        ...
        const Spacer(),
        Text(
          _formatedToMoneyWithCurrency(
              symbol?.trader, deal.closePositionDetail!.grossProfit),
          style: THEME.texts.bodyMediumSecondary.copyWith(
              color: deal.closePositionDetail!.grossProfit < 0
                  ? THEME.red
                  : THEME.green),
        ),
        const SizedBox(width: 16),
      ...
      Row(children: <Widget>[
        ...
        Text(
          _formatedToMoneyWithCurrency(
              symbol?.trader, deal.closePositionDetail!.commission),
          style: THEME.texts.bodyMediumSecondary,
        ),
        const SizedBox(width: 16),
      ]),
      Row(children: <Widget>[
        ...
        Text(
          _formatedToMoneyWithCurrency(
              symbol?.trader, deal.closePositionDetail!.swap),
          style: THEME.texts.bodyMediumSecondary,
        ),
        const SizedBox(width: 16),
      ]),
      ...
      Row(children: <Widget>[
        ...
        const Spacer(),
        Text(
          symbol?.details?.formattedRate(humanic: deal.executionPrice) ??
  ...
  String _formatedToMoneyWithCurrency(TraderData? trader, int cents) {
    return trader?.formattedMoneyWithCurrency(cents: cents) ??
        (cents / 100).toComaSeparated(decimals: 2);
  }
  ...
  Future<void> _loadMoreDeals([int loaded = 0, int reqNum = 1]) async {
    ...
    final int fromTs =
        max(_timestampOfLastDeal - 604800000, trader.registretionTs);
    if (fromTs >= _timestampOfLastDeal) {
      final AppLocalizations l10n = AppLocalizations.of(context)!;
      _hasMoreDeals = false;
      GetIt.I<PopupManager>().showPopup(
          title: l10n.tradingHistory, message: l10n.allHistoryLoaded);
      GetIt.I<AppState>().setUIBlocked(false);
      return;
    }

    final ProtoOADealListRes resp =
        await remoteAPI.sendDealList(trader.id, fromTs, _timestampOfLastDeal);
    ...
    for (final ProtoOADeal d in resp.deal) {
      if (d.closePositionDetail != null) {
        _deals.add(d);
        loaded++;
        if (trader.tree.symbol(d.symbolId)?.details == null)
          _symbolsToLoad.add(d.symbolId);
      }
      lastTS = min(lastTS, d.executionTimestamp);
    }

    if (_symbolsToLoad.isNotEmpty)
      remoteAPI.sendSymbolById(trader.id, _symbolsToLoad.toList());
    ...
    if (loaded < 50 && reqNum < 5) {
      ...
      if (loaded == 0) {
        GetIt.I<PopupManager>().showPopup(
            title: l10n.tradingHistory, message: l10n.noRecentHistory);
      }
    }
  ...
  Future<void> _onToggleDealExpend(int dealId) async {
    ...
    if (!_expandedDeals.remove(dealId)) {
      ...
      if (!_dealOffsets.containsKey(dealId)) {
        final ProtoOADealOffsetListRes resp =
            await trader.remoteApi.sendDealOffsetList(trader.id, dealId);
