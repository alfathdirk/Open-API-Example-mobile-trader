...
import 'package:myfxhistory_app/widgets/block_login_ui.dart';
import 'package:myfxhistory_app/widgets/button_primary.dart';
import 'package:myfxhistory_app/widgets/login_bottom_banner.dart';
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void initState() {
    ...
    if (!GetIt.I<UserState>().isLogining &&
        GetIt.I<UserState>().atJSONString != null) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: <Widget>[
                ...
                child: Column(
                  children: <Widget>[
                    ...
                    Padding(
                      ...
                      child: Text(l10n.loginSignInLable,
                          style: THEME_LOGIN.texts.headingStrong,
                    ...
                    Text(l10n.dontHaveAnAccoutnYet,
                        style: THEME_LOGIN.texts.headingStrong),
                    ...
                    _renderBroker(
                        context,
                        l10n.continueWithBroker('IC Markets'),
                        'ic_markets',
                        'https://www.icmarkets.com/open-new-live-account/?camp=3263'),
                    _renderBroker(
                        context,
                        l10n.continueWithBroker('Pepperstone'),
                        'pepperstone',
                        'https://track.pepperstonepartners.com/visit/?bta=35600&nci=5399'),
                    _renderBroker(
                        context,
                        l10n.continueWithBroker('TopFx'),
                        'top_fx',
                        'https://signup.topfx.com.sc/Registration/Main/Account?dest=live&isSpecAts=true&camp=7087'),
                    _renderBroker(
                        context,
                        l10n.continueWithBroker('FxPro'),
                        'fx_pro',
                        'https://direct.fxpro.com.cy/partner/8033201'),
          ...
          if (_isBlocked)
            Positioned.fill(child: BlockLoginUI(label: _blockUiLabel)),
        ],
      ),
    );
  }

  Widget _renderBroker(
      BuildContext context, String broker, String icon, String link) {
    return Padding(
    ...
    try {
      ...
      if (at == null) {
        final String? authCode =
            await Navigator.pushNamed(context, LoginWebView.ROUTE_NAME);
        ...
        if (jsonResp['errorCode'] != null) {
          GetIt.I<PopupManager>().showError(AppLocalizations.of(context)!,
              '${jsonResp['errorCode']} ${jsonResp['description']}');
      ...
      if (!demoAPI.isAutorized) {
        ...
        GetIt.I<PopupManager>().showPopup(
            title: l10n.errorOccurred, message: l10n.someErrorOccuredTryLater);
        setState(() => _isBlocked = false);
        return;
      }

      try {
        accountsResp = await demoAPI
            .getAccountListByAccessTokenReq(jsonAT['accessToken'] as String);
        userState.cacheAccountToken(accountsResp.accessToken);
      } catch (e) {
        if (e is proto.ProtoOAErrorRes &&
            e.errorCode == 'CH_ACCESS_TOKEN_INVALID') {
      ...
      if (accountsResp.ctidTraderAccount.isEmpty) {
        final AppLocalizations l10n = AppLocalizations.of(context)!;
        GetIt.I<PopupManager>().showPopup(
            title: l10n.noAccounts, message: l10n.noAccountsFoundVerifyEmail);
        throw 'NO_ACCESS';
      }

      for (final proto.ProtoOACtidTraderAccount traderAcc
          in accountsResp.ctidTraderAccount) {
        if (ONLY_DEMO && traderAcc.isLive == true) continue;

        setState(() => _blockUiLabel = AppLocalizations.of(context)!
            .loadingDataForAcc(traderAcc.traderLogin ?? 0));
        final RemoteApi traderRemoteAPI =
            traderAcc.isLive == true ? liveAPI : demoAPI;
        try {
          ...
          if (e is proto.ProtoOAErrorRes) {
            ...
            Logger.error(
                'Error occurred at trader account authenthification', e);
          }
          continue;
        }

        final proto.ProtoOATraderRes traderResp =
            await traderRemoteAPI.sendTraderReq(traderAcc.ctidTraderAccountId);

        userState.addTrader(traderAcc.isLive == true, traderResp.trader);

        final TraderData trader =
            userState.trader(traderAcc.ctidTraderAccountId)!;
        trader.name = traderAcc.brokerTitleShort;

        final proto.ProtoOAAssetListRes assetsResp =
            await traderRemoteAPI.sendAssetListReq(trader.id);
        trader.tree.handleAssetsResponse(assetsResp.asset);

        final proto.ProtoOAAssetClassListRes assetClassResp =
            await traderRemoteAPI.sendAssetClassListReq(trader.id);
        trader.tree.handleAssetClassResponse(assetClassResp.assetClass);

        final proto.ProtoOASymbolCategoryListRes symbolCategoryResp =
            await traderRemoteAPI.sendSymbolCategoryListReq(trader.id);
        trader.tree
            .handleSymbolCategoryResponse(symbolCategoryResp.symbolCategory);

        final proto.ProtoOASymbolsListRes symbolsResp =
            await traderRemoteAPI.sendSymbolsListReq(trader.id);
      ...
      if (userState.traders.isEmpty && ONLY_DEMO) {
        setState(() => _isBlocked = false);
        final AppLocalizations l10n = AppLocalizations.of(context)!;
        await GetIt.I<PopupManager>().showPopup(
            title: l10n.demoAccountRequired,
            message: l10n.loginWithDemoAccount);
        logout(context);
        return;
        ...
        setState(() => _isBlocked = false);
        GetIt.I<PopupManager>().showPopup(
            title: l10n.noAccess, message: l10n.noAccessToTradingContactBroker);
        throw 'NO_ACCESS';
      ...
      if (!appState.moreAccountsWarnPopupShowed &&
          (userState.traders.length > 1 ||
              trader.accessRights != proto.ProtoOAAccessRights.fullAccess)) {
        appState.setMoreAccountsWarnPopupShowed();
        await GetIt.I<PopupManager>().showTraderSelection(
      ...
      if (err != 'SOCKET_CLOSED' && err != 'NO_ACCESS')
        GetIt.I<PopupManager>()
            .showSomeErrorOccurred(AppLocalizations.of(context)!);
    }
