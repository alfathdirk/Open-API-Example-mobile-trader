...
class _ActivityPositionsState extends State<ActivityPositions> {
  ...
  void didChangeDependencies() {
    ...
    for (final PositionData position in trader.positionsManager.activities) {
      if (position.symbol?.details == null)
        symbolsToLoad.add(position.symbolId);
    }

    if (symbolsToLoad.isNotEmpty)
      trader.remoteApi.sendSymbolById(trader.id, symbolsToLoad.toList());
  ...
  Widget build(BuildContext context) {
    ...
    final List<PositionData> positions =
        trader.positionsManager.activities.toList();

    if (positions.isEmpty) {
      return Container(
        padding: const EdgeInsets.all(16),
        child: Text(l10n.noActivePositions,
            style: THEME.texts.headingBold, textAlign: TextAlign.center),
      );
    } else {
      positions.sort(
          (PositionData a, PositionData b) => b.opened.compareTo(a.opened));
  ...
  Widget _positionItem(
      AppLocalizations l10n, PositionData position, SymbolData? symbol) {
          ...
          GestureDetector(
            ...
            child: Row(children: <Widget>[
              ...
              Container(
                ...
                child: Text(position.isBuy ? l10n.buy : l10n.sell,
              ...
              Text(
                symbol?.trader
                    ...
                    (position.netPnl / 100).toComaSeparated(decimals: 2),
                style: THEME.texts.bodyBold.copyWith(
                    color: position.netPnl < 0 ? THEME.red : THEME.green),
  ...
  List<Widget> _positionDetails(AppLocalizations l10n, PositionData position) {
    ...
    if (position.trailingStopLoss) {
      ...
      if (trailingStopDist != null) {
        trailingStop =
            position.symbol?.details?.formattedPips(system: trailingStopDist) ??
    ...
    return <Widget>[
      ...
      Row(children: <Widget>[
        ...
        const Spacer(),
        Text(position.formattedVolumeWithUnits,
      ...
      Row(children: <Widget>[
        ...
        const Spacer(),
        Text(position.formattedRate(system: position.rate),
            style: THEME.texts.bodyMediumSecondary),
        const SizedBox(width: 16),
      ]),
      Row(children: <Widget>[
        ...
        Text(
          position.takeProfit == null
              ? l10n.no
              : position.formattedRate(system: position.takeProfit),
          style: THEME.texts.bodyMediumSecondary,
        ),
        const SizedBox(width: 16),
      ]),
      if (!position.trailingStopLoss)
        Row(children: <Widget>[
          const SizedBox(width: 16),
          Text(
            position.garanteedStopLoss
                ? l10n.guaranteedStopLoss
                : l10n.stopLoss,
            style: THEME.texts.bodyMediumSecondary,
          ),
          const Spacer(),
          Text(
            position.stopLoss == null
                ? l10n.no
                : position.formattedRate(system: position.stopLoss),
            style: THEME.texts.bodyMediumSecondary,
          ),
          const SizedBox(width: 16),
        ]),
  ...
  Future<void> _onTapClosePosition(
    ...
    } else if (position.symbol?.details?.data.tradingMode ==
  ...
  void _onTapEditPosition(PositionData position) {
    if (GetIt.I<UserState>().selectedTrader.accessRights.index >=
      ...
      Navigator.pushNamed(context, EditOrderPositionScreen.ROUTE_NAME,
          arguments: <String, dynamic>{'positionId': position.id});
    }
  }
}
