...
class _TradingRoomScreenState extends State<TradingRoomScreen> {
  ...
  void initState() {
    ...
    GetIt.I<UserState>()
        .selectedTrader
        .remoteApi
        .subscribe<ProtoOASpotEvent>(_onHandleSpotEvent);
    ...
    _controller = WebViewController()
      ...
      ..addJavaScriptChannel('native',
        ...
        if (event['type'] == 'loadCandles') {
          ...
          _loadCandlesForChart(
              symbol, timeframe, count, callback, timestampFrom);
    ...
    Timer.run(() async {
      ...
      if (_chartPeriod != null && _chartSymbol != null)
        await spotSubscriptionManager.unsubscribeFromTrendBars(
            trader.id, _chartSymbol!.id, _chartPeriod!);
      if (_chartSymbol != null)
        await spotSubscriptionManager
            .unsubscribe(trader.id, <int>[_chartSymbol!.id]);
    });
          ...
          SizedBox(
            ...
            child: Row(children: <Widget>[
              ...
              Expanded(
                  child: BuySellButton(
                      false,
                      _chartSymbol?.formattedSellRate ?? '----',
                      _onClickBuySellButton)),
              const SizedBox(width: 16),
              Expanded(
                  child: BuySellButton(
                      true,
                      _chartSymbol?.formattedBuyRate ?? '----',
                      _onClickBuySellButton)),
              const SizedBox(width: 16),
            ]),
          ),
    ...
    if (_chartPeriod != null && _chartSymbol != null) {
      if (event.trendbar != null && event.trendbar!.isNotEmpty) {
        ...
        if (trendbar.period == _chartPeriod) {
          ...
          final String msg =
              "chart.data.setCandle('$_chartSymbolName', '${_chartPeriod!.toChartName()}',"
  ...
  Future<void> _loadCandlesForChart(
      String symbolName, String timeframe, int count, String callback,
      [int? timestampFrom]) async {
    ...
    final Iterable<SymbolData> symbols =
        trader.tree.findSymbolForSimultaneousTrading(name: symbolName);
    ...
    final int to = timestampFrom != null
        ? timestampFrom * 1000
    ...
    try {
      ...
      await spotSubscriptionManager.subscribeForTrendBars(
          trader.id, symbol.id, period);

      final ProtoOAGetTrendbarsRes trendbarsRes = await trader.remoteApi
          .sendGetTrendbars(trader.id, symbol.id, count, period, from, to);
  ...
  Future<void> _symbolChanged(String symbolName) async {
    ...
    if (_chartSymbol != null && _chartPeriod != null) {
      try {
        ...
        Logger.error(
            'Error occurred for unsubscibing from live trend bars for symbol(${_chartSymbol!.name})',
            e);
      }
    }
    ...
    if (_chartSymbol != null) {
      try {
        ...
        Logger.error(
            'Error occurred for unsubscribing from spots for symbol(${_chartSymbol!.name})',
            e);
      } finally {
        _chartSymbol = null;
        _chartSymbolName = null;
    ...
    if (symbols.isEmpty) {
      setState(() {});
      return Logger.error(
          'Symbol($symbolName) is not found for selected trader');
    }
    ...
    try {
      ...
      await spotSubscriptionManager.subscribeForTrendBars(
          trader.id, symbol.id, _chartPeriod!);
    } catch (e) {
  ...
  void _onClickBuySellButton(bool isBuy) {
    if (_chartSymbol != null) {
      Navigator.pushNamed(context, BuySellScreen.ROUTE_NAME,
          arguments: <String, Object>{
            'isBuy': isBuy,
            'symbolId': _chartSymbol!.id
          });
    }
  }
}
