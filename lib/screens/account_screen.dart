...
import 'package:myfxhistory_app/screens/market_screen.dart';
...
import 'package:myfxhistory_app/widgets/wrapped_checkbox.dart';
import 'package:myfxhistory_app/widgets/wrapped_dropdown.dart';
...
class _AccountScreenState extends State<AccountScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final int selectedTraderIndex = _traders
        .indexWhere((TraderData t) => t.id == userState.selectedTraderId);
          ...
          onHorizontalDragUpdate: (DragUpdateDetails details) {
            if (details.delta.dx < -20) {
              changePageWithSlideTransition(
                  context, const Offset(1, 0), const ActivityScreen());
            } else if (details.delta.dx > 20) {
              changePageWithSlideTransition(
                  context, const Offset(-1, 0), const MarketScreen());
            }
          },
                ...
                Expanded(
                  child: ListView(
                    children: <Widget>[
                      ...
                      if (_isAllowSimultaneousTrading)
                        _simultaneousSection(l10n),
                      _divider(),
                    ],
                  ),
                ),
  ...
  Widget _selectAccountDropdownLine(
      AppLocalizations l10n, int selectedTraderIndex) {
    return Container(
      height: 68,
        ...
        children: <Widget>[
          ...
          Expanded(
            child: WrappedDropdown(
              ...
              items: _traders
                  .map((TraderData t) =>
                      '${t.demo ? l10n.demo : l10n.live}: ${t.login} - ${t.name}')
                  .toList(),
  ...
  Widget _accountInfo(AppLocalizations l10n, TraderData trader) {
    final Color? pnlColor =
        trader.pnl > 0 ? THEME.green : (trader.pnl < 0 ? THEME.red : null);

    return Stack(
      children: <Widget>[
        Column(children: <Widget>[
          const SizedBox(height: 8),
          _accountInfoLine(l10n.balanceLabel,
              trader.formattedMoneyWithCurrency(cents: trader.balance)),
          _accountInfoLine(l10n.equityLabel,
              trader.formattedMoneyWithCurrency(cents: trader.equity)),
          _accountInfoLine(l10n.marginLabel,
              trader.formattedMoneyWithCurrency(cents: trader.margin)),
          _accountInfoLine(l10n.freeMarginLabel,
              trader.formattedMoneyWithCurrency(cents: trader.freeMargin)),
          _accountInfoLine(
              l10n.marginLevelLabel, '${trader.formattedMarginLevel}%'),
          _accountInfoLine(l10n.unrNetPnlLabel,
              trader.formattedMoneyWithCurrency(cents: trader.pnl), pnlColor),
          const SizedBox(height: 8),
        ]),
      ],
    );
    ...
    return Container(
      ...
      child: Row(
        children: <Widget>[
          ...
          const Spacer(),
          Text(value,
              style: THEME.texts.bodyMediumSecondary.copyWith(color: color)),
        ],
      ),
    );
  }
      ...
      child: Row(
        children: <Widget>[
          ...
          WrappedSwitch(
            ...
            onChange: (bool v) =>
                appState.selectTheme(v ? ThemeType.dark : ThemeType.light),
          )
        ],
      ),
    );
      ...
      child: Row(children: <Widget>[
        ...
        Container(
          ...
          child: WrappedSwitch(
            ...
            onChange: (bool v) =>
                setState(() => appState.setShowOrdersInMarkets(v)),
          ),
        ),
      ]),
    );
  ...
  Widget _showAllowSimulteneousTradingSelectorLine(AppLocalizations l10n) {
    return Container(
      ...
      child: Row(children: <Widget>[
        Expanded(
            child: Text(l10n.allowSimultaneousTradingForAccounts,
                style: THEME.texts.headingBold)),
        Container(
          width: 46,
          alignment: Alignment.centerRight,
  ...
  Widget _simultaneousSection(AppLocalizations l10n) {
    return Column(children: <Widget>[
      if (GetIt.I<SimultaneousTrdaingState>().enabled)
        Container(
          ...
          child: Row(
            children: <Widget>[
              Text(
                l10n.allowdForAmountAccounts(
                    GetIt.I<SimultaneousTrdaingState>().pariedAccounts.length),
      ...
      for (final TraderData acc in _simultaniousAccounts)
        _simultaneousAccountLine(l10n, acc),
            ...
            children: <Widget>[
              ...
              Flexible(
                ...
                child: GestureDetector(
                  onTap: () => GetIt.I<PopupManager>().showPopup(
                      message:
                          '${l10n.simultaniousTradingTooltip}\n\n${l10n.simultaniousTradingTooltipNote}\n'),
                  child: Container(
                    alignment: Alignment.centerLeft,
                    color: Colors.transparent,
                    child: Container(
        ...
        children: <Widget>[
          Expanded(
            ...
            Container(
              ...
              child: Text(account.demo ? l10n.demo : l10n.live,
                  style: THEME.texts.bodyBold.copyWith(color: Colors.white)),
            ),
            const SizedBox(width: 16),
            Flexible(
              flex: 2,
  ...
  Future<void> _toggleSimulteniousTradingSelector(bool enable) async {
    if (enable &&
        GetIt.I<UserState>().selectedTrader.accessRights !=
            ProtoOAAccessRights.fullAccess) {
      final AppLocalizations l10n = AppLocalizations.of(context)!;
    ...
    if (enable) {
      ...
      if (userState.hasBlockedAccounts &&
          !userState.blockedAccountsWarningPopup) {
        ...
        GetIt.I<PopupManager>().showPopup(
          title: l10n.noAccess,
          message: l10n.accountsHasNoAccessToTradingContactBroker(
                  userState.blockedAccounts.length) +
              accounts.toString(),
        );
      ...
      for (final TraderData acc in userState.traders) {
        if (acc.accessRights == ProtoOAAccessRights.fullAccess)
          _simultaniousAccounts.add(acc);
      }
        ...
        final PopupResult result = await GetIt.I<PopupManager>().showPopup(
          ...
          buttons: <Widget>[
            ButtonSecondary(
              ...
              onTap: () =>
                  GetIt.I<PopupManager>().currentPopup?.closeWithResult(false),
            ),
            ...
            ButtonPrimary(
              ...
              height: popup_constants.singleButtonSize.height,
              onTap: () =>
                  GetIt.I<PopupManager>().currentPopup?.closeWithResult(true),
            ),
          ],
        );
      ...
      if (simultaneousState.hasPairedOrders ||
          simultaneousState.hasPairedPosition) {
        final PopupResult result = await GetIt.I<PopupManager>()
            .askToDisableSimulataneousTrading(AppLocalizations.of(context)!);
  ...
  void _onChangeAccount(int? index) {
    ...
    if (trader.accountType == ProtoOAAccountType.spreadBetting ||
        trader.accessRights != ProtoOAAccessRights.fullAccess) {
      GetIt.I<PopupManager>()
          .showTraderSelection(AppLocalizations.of(context)!, trader, false);
    }
    if (trader.accountType != ProtoOAAccountType.spreadBetting &&
        trader.accessRights != ProtoOAAccessRights.noLogin) {
  ...
  Future<void> _onTapSaveSimultaneous() async {
    ...
    if (_simultaniousSelectedAccounts.length < 2) {
      GetIt.I<PopupManager>().showPopup(
          title: l10n.incorrectChoice,
          message: l10n.simultaneousChoose2Accounts);
      return;
    }
    ...
    if (removedAccounts.isNotEmpty &&
        removedAccounts.any((int id) =>
            simultaneousState.isAccountHasPaierdOrders(id) ||
            simultaneousState.isAccountHasPaierdPositions(id))) {
      final PopupResult result =
          await GetIt.I<PopupManager>().askToDisableSimulataneousTrading(l10n);
      if (!result.agree) {
        _simultaniousSelectedAccounts.clear();
        _simultaniousSelectedAccounts
            .addAll(GetIt.I<SimultaneousTrdaingState>().pariedAccounts);
        setState(() {});
        return;
      }
    ...
    final Set<ProtoOAAccountType?> selectedAccountTypes =
        <ProtoOAAccountType?>{};
    ...
    if (selectedAccountTypes.length > 1) {
      GetIt.I<PopupManager>().showPopup(
          title: l10n.warning,
          message: l10n.simultaneousIncludeBothAccountTypes);
    }
  ...
  void _onTapEditSimultaneousList() {
    ...
    if (userState.selectedTrader.accessRights !=
        ProtoOAAccessRights.fullAccess) {
      final AppLocalizations l10n = AppLocalizations.of(context)!;
