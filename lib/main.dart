...
import 'package:myfxhistory_app/screens/login_screen.dart';
import 'package:myfxhistory_app/screens/login_webview.dart';
import 'package:myfxhistory_app/screens/market_screen.dart';
import 'package:myfxhistory_app/screens/trading_room_screen.dart';
...
import 'package:myfxhistory_app/styles/dark_theme_config.dart';
import 'package:myfxhistory_app/styles/itheme_config.dart';
import 'package:myfxhistory_app/styles/light_theme_config.dart';
import 'package:myfxhistory_app/styles/login_theme.dart';
import 'package:myfxhistory_app/widgets/block_widget.dart';
...
final List<IThemeConfig> THEMES = <IThemeConfig>[
  LightThemeConfig(),
  DarkThemeConfig()
];
...
void main() async {
  ...
  FlutterError.onError = (FlutterErrorDetails details) {
    Logger.error(
        ' UNHANDELED FLUTTER ERROR '.centerLabel(80, '='), details.exception);
  ...
  final String jsonStr =
      await rootBundle.loadString('assets/json/symbol_pairs.json');
  ...
  GetIt.I
      .registerSingleton<SimultaneousTrdaingState>(SimultaneousTrdaingState());
  ...
  GetIt.I<RemoteAPIManager>().subscribe<proto.ProtoOASymbolByIdRes>(
      GetIt.I<UserState>().handleSymbolDetailsResponse);
  GetIt.I<RemoteAPIManager>()
      .subscribe<proto.ProtoOAExecutionEvent>(_handleExecutionEvent);
  GetIt.I<RemoteAPIManager>().subscribe<proto.ProtoOATrailingSLChangedEvent>(
      GetIt.I<UserState>().handlePositionTrailingStopEvent);
  GetIt.I<RemoteAPIManager>()
      .subscribe<proto.ProtoOASpotEvent>(GetIt.I<UserState>().handleSpotEvent);
  GetIt.I<RemoteAPIManager>()
      .subscribe<proto.ProtoOAGetPositionUnrealizedPnLRes>(
          GetIt.I<UserState>().handlePnlEvent);
  GetIt.I<RemoteAPIManager>().subscribe<proto.ProtoOAReconcileRes>(
      GetIt.I<UserState>().handleReconcileResponse);
  GetIt.I<RemoteAPIManager>().subscribe<proto.ProtoOASymbolChangedEvent>(
      GetIt.I<UserState>().handleSymbolChangedEvent);
  GetIt.I<RemoteAPIManager>().subscribe<proto.ProtoOATraderUpdatedEvent>(
      GetIt.I<UserState>().handleTraderUpdatedEvent);
  GetIt.I<RemoteAPIManager>().subscribe<proto.ProtoOAMarginChangedEvent>(
      GetIt.I<UserState>().handleMarginChangedEvent);
...
void _handleExecutionEvent(proto.ProtoOAExecutionEvent event) {
  ...
  if (!GetIt.I<SimultaneousTrdaingState>()
          .isAccountPaired(event.ctidTraderAccountId) &&
      selectedTraderId != event.ctidTraderAccountId) return;

  if (event.order == null) return;

  final AppLocalizations l10n =
      AppLocalizations.of(NAVIGATOR_KEY.currentContext!)!;
  final proto.ProtoOAOrder oaOrder = event.order!;
  final bool isMarket = oaOrder.orderType == proto.ProtoOAOrderType.market;
  final bool isOrder = oaOrder.orderType == proto.ProtoOAOrderType.limit ||
      oaOrder.orderType == proto.ProtoOAOrderType.stop;
  final bool isSlTp =
      oaOrder.orderType == proto.ProtoOAOrderType.stopLossTakeProfit;
  final bool isFilled =
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: <SingleChildWidget>[
        ChangeNotifierProvider<PopupManager>(
            create: (BuildContext context) => GetIt.I<PopupManager>()),
        ChangeNotifierProvider<AppState>(
            create: (BuildContext context) => GetIt.I<AppState>()),
        ChangeNotifierProvider<UserState>(
            create: (BuildContext context) => GetIt.I<UserState>()),
        ChangeNotifierProvider<TutorialState>(
            create: (BuildContext context) => GetIt.I<TutorialState>()),
      ],
          ...
          onGenerateRoute: (RouteSettings settings) {
            switch (settings.name) {
              ...
              case TradingRoomScreen.ROUTE_NAME:
                ...
                return MaterialPageRoute<void>(
                    builder: (BuildContext context) =>
                        const TradingRoomScreen());

              case LoginWebView.ROUTE_NAME:
                return MaterialPageRoute<String?>(
                    builder: (BuildContext context) => const LoginWebView());
              ...
              case BuySellScreen.ROUTE_NAME:
                ...
                return MaterialPageRoute<void>(
                    builder: (BuildContext context) =>
                        BuySellScreen(isBuy: isBuy, symbolId: symbolId));
              ...
              case EditOrderPositionScreen.ROUTE_NAME:
                ...
                return MaterialPageRoute<void>(
                    builder: (BuildContext context) => EditOrderPositionScreen(
                        orderId: orderId, positionId: positionId));
            }
