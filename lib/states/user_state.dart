...
import 'package:myfxhistory_app/models/trailing_stop_values.dart';
import 'package:myfxhistory_app/remote/proto.dart' as proto;
import 'package:flutter/material.dart';
import 'package:flutter_secure_storage/flutter_secure_storage.dart';
import 'package:shared_preferences/shared_preferences.dart';

enum UserStatePrefKeys {
  dontShowReversOrderPopup,
  dontShowSimulTradingOpenPopup,
  dontShowSimulTradingEdfitPopup,
  trailingStopValues
}
...
class UserState extends ChangeNotifier {
  ...
  Future<void> _saveState() async {
    ...
    prefs.setBool(UserStatePrefKeys.dontShowReversOrderPopup.toString(),
        _dontShowReversOrderPopup);
    prefs.setBool(UserStatePrefKeys.dontShowSimulTradingOpenPopup.toString(),
        _dontShowSimulTradingOpenPopup);
    prefs.setBool(UserStatePrefKeys.dontShowSimulTradingEdfitPopup.toString(),
        _dontShowSimulTradingEditPopup);
    prefs.setString(UserStatePrefKeys.trailingStopValues.toString(),
        jsonEncode(_trailingStopValues));
  }

  Future<void> _resotreState() async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    _dontShowReversOrderPopup =
        prefs.getBool(UserStatePrefKeys.dontShowReversOrderPopup.toString()) ==
            true;
    _dontShowSimulTradingOpenPopup = prefs.getBool(
            UserStatePrefKeys.dontShowSimulTradingOpenPopup.toString()) ==
        true;
    _dontShowSimulTradingEditPopup = prefs.getBool(
            UserStatePrefKeys.dontShowSimulTradingEdfitPopup.toString()) ==
        true;
    if (prefs.containsKey(UserStatePrefKeys.trailingStopValues.toString())) {
      final String json =
          prefs.getString(UserStatePrefKeys.trailingStopValues.toString())!;
  ...
  bool get dontShowSimultaneousTradingPopupForNew =>
      _dontShowSimulTradingOpenPopup;
  ...
  bool get dontShowSimultaneousTradingPopupForEdit =>
      _dontShowSimulTradingEditPopup;
  ...
  Iterable<TraderData> get demoTraders =>
      _traders.values.where((TraderData trader) => trader.demo);
  Iterable<TraderData> get liveTraders =>
      _traders.values.where((TraderData trader) => !trader.demo);
  TraderData? trader(int id) => _traders[id];
  void addTrader(bool isLive, proto.ProtoOATrader trader) {
    _traders[trader.ctidTraderAccountId] =
        TraderData.parseResponse(!isLive, trader);
    notifyListeners();
  }
  ...
  void selectTrader(int? traderId) {
    traderId ??= _traders.keys.fold(0,
        (int? previousValue, int element) => max(previousValue ?? 0, element));
  ...
  final List<proto.ProtoOACtidTraderAccount> _blockedAccounts =
      <proto.ProtoOACtidTraderAccount>[];
  bool get hasBlockedAccounts => _blockedAccounts.isNotEmpty;
  Iterable<proto.ProtoOACtidTraderAccount> get blockedAccounts =>
      _blockedAccounts;
  ...
  Future<void> _restoreBlockedAccountsState() async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    _blockedAccountsWarningPopup =
        prefs.getBool('blockedAccountsWarningPopup') == true;
  }
  ...
  void handleSpotEvent(proto.ProtoOASpotEvent event) {
    trader(event.ctidTraderAccountId)
        ?.tree
  ...
  void handlePnlEvent(proto.ProtoOAGetPositionUnrealizedPnLRes event) {
    ...
    for (final proto.ProtoOAPositionUnrealizedPnL p
        in event.positionUnrealizedPnL) {
      final PositionData? position =
          trader.positionsManager.activityBy(id: p.positionId);
      if (position != null &&
          position.updatePnl(p.grossUnrealizedPnL, p.netUnrealizedPnL)) {
        hasUpdates = true;
  ...
  void handleExecutionEvent(proto.ProtoOAExecutionEvent event) {
    ...
    final bool isMarket =
        event.order?.orderType == proto.ProtoOAOrderType.market;
    final bool isSLTP =
        event.order?.orderType == proto.ProtoOAOrderType.stopLossTakeProfit;
    final bool isOrder =
        event.order?.orderType == proto.ProtoOAOrderType.limit ||
            event.order?.orderType == proto.ProtoOAOrderType.stop;

    if (event.executionType == proto.ProtoOAExecutionType.depositWithdraw) {
      _handleDepositWithdrawEvent(trader, event);
    } else if (event.executionType ==
        proto.ProtoOAExecutionType.bonusDepositWithdraw) {
      ...
      _handleMarketExecutionEvent(trader, event);
        ...
        (event.executionType == proto.ProtoOAExecutionType.orderFilled ||
            event.executionType ==
                proto.ProtoOAExecutionType.orderPartialFill)) {
  ...
  Future<void> handleSymbolChangedEvent(
      proto.ProtoOASymbolChangedEvent event) async {
    final TraderData? trader = this.trader(event.ctidTraderAccountId);
    if (trader == null) return;
  ...
  void _handleDepositWithdrawEvent(
      TraderData trader, proto.ProtoOAExecutionEvent event) {
    final proto.ProtoOADepositWithdraw oaDepositWithdraw =
        event.depositWithdraw!;
    if (trader.updateBalance(
        oaDepositWithdraw.balanceVersion ?? 0, oaDepositWithdraw.balance))
      notifyListeners();
  }

  void _handleBonusDepositWithdrawEvent(
      TraderData trader, proto.ProtoOAExecutionEvent event) {
    final proto.ProtoOABonusDepositWithdraw oaBonusDepositWithdraw =
        event.bonusDepositWithdraw!;
    trader.updateBonuses(
        oaBonusDepositWithdraw.ibBonus, oaBonusDepositWithdraw.managerBonus);
    notifyListeners();
  }

  void _handleMarketExecutionEvent(
      TraderData trader, proto.ProtoOAExecutionEvent event) {
    final proto.ProtoOAOrder oaOrder = event.order!;
    final proto.ProtoOAPosition oaPosition = event.position!;
    final bool isPartial =
        event.executionType == proto.ProtoOAExecutionType.orderPartialFill;

    if (event.executionType == proto.ProtoOAExecutionType.orderAccepted ||
        event.executionType == proto.ProtoOAExecutionType.orderCancelled) {
      return;
    } else if (oaOrder.isStopOut == true) {
      if (oaPosition.tradeData.volume == 0) {
        trader.removePosition(
            oaPosition.positionId, event.deal?.closePositionDetail);
      } else {
      ...
      trader.removePosition(
          oaPosition.positionId, event.deal?.closePositionDetail);
    } else if (event.executionType == proto.ProtoOAExecutionType.orderFilled ||
        event.executionType == proto.ProtoOAExecutionType.orderPartialFill) {
      if (trader.accountType == proto.ProtoOAAccountType.hedged) {
        ...
        trader.removePosition(
            oaPosition.positionId, event.deal?.closePositionDetail);
      } else if (oaOrder.executedVolume == oaPosition.tradeData.volume &&
          oaOrder.tradeData.tradeSide == oaPosition.tradeData.tradeSide) {
        trader.addPosition(PositionData.parseResponse(trader, oaPosition));
      } else {
        trader.updatePosition(oaPosition);
      }
  ...
  void _handleSLTPExecutionEvent(
      TraderData trader, proto.ProtoOAExecutionEvent event) {
    final proto.ProtoOAPosition oaPosition = event.position!;
  ...
  void _handlePendingOrderExecuteEvent(
      TraderData trader, proto.ProtoOAExecutionEvent event) {
    _handleMarketExecutionEvent(trader, event);
  ...
  void _handleOrderExecutionEvent(
      TraderData trader, proto.ProtoOAExecutionEvent event) {
    final proto.ProtoOAOrder oaOrder = event.order!;
    ...
    if (event.executionType == proto.ProtoOAExecutionType.orderCancelled ||
      ...
      trader.ordersManager.removeActivity(oaOrder.orderId);
        ...
        proto.ProtoOAExecutionType.orderAccepted) {
      trader.ordersManager
          .addActivity(OrderData.parseResponse(trader, oaOrder));
    } else {
      trader.ordersManager.updateActivity(oaOrder.orderId, oaOrder);
    }
  }

  void handlePositionTrailingStopEvent(
      proto.ProtoOATrailingSLChangedEvent event) {
  ...
  bool isSymbolFavorite(int symbolId) =>
      selectedTrader.tree.isFavorite(symbolId);
  ...
  Future<void> _saveTraderFavorites(int traderId) async {
    ...
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setString('${traderId}_favorites',
        jsonEncode(trader.tree.favoriteSymbolIds.toList()));
  }
  ...
  Future<void> restoreTraderFavorites(int traderId) async {
    ...
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    final dynamic idsJson =
        jsonDecode(prefs.getString('${traderId}_favorites') ?? '[]');
  ...
  Future<void> _saveCollapsedCategories(int traderId) async {
    ...
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setString('${traderId}_collapsedCategories',
        jsonEncode(trader.tree.collapsedCategories.toList()));
  }
  ...
  Future<void> restoreCollapsedCategories(int traderId) async {
    ...
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    final dynamic idsJson =
        jsonDecode(prefs.getString('${traderId}_collapsedCategories') ?? '[]');
    final List<int> ids = List.castFrom<dynamic, int>(idsJson as List<dynamic>);
    trader.tree.restoreCollapsedCategories(ids);
  }

  Future<void> saveSymbolVolume(
      {required int traderId, required int symbolId, required int volume}) {
    return SharedPreferences.getInstance().then((SharedPreferences prefs) =>
        prefs.setInt('volume_${traderId}_$symbolId', volume));
  }

  Future<int?> resotreSymbolVolume(
      {required int traderId, required int symbolId}) async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
