...
class RemoteApi {
  RemoteApi(this.url, this.isDemo)
      : logTag = 'Remote.${isDemo ? 'demo' : 'live'}' {
    subscribe<ProtoHeartbeatEvent>(_handleHeartBeatEvent);

    Timer.periodic(
        const Duration(seconds: 1), (_) => _checkWaitersForTimeout());
    Timer.periodic(const Duration(seconds: 28),
        (Timer timer) => _isAutorized ? _send(ProtoHeartbeatEvent()) : null);
  }
  ...
  final Map<String, Queue<RemoteAPIWaiter>> _oneShotWaiters =
      <String, Queue<RemoteAPIWaiter>>{};
  final Set<int> _autorizedAccounts = <int>{};
  final Queue<Completer<void>> _appAutorizationWaiters =
      Queue<Completer<void>>();
  final Map<int, Queue<Completer<void>>> _accountAutorizationWaiters =
      <int, Queue<Completer<void>>>{};
  ...
  String _send(ProtoMessage message) {
    ...
    if (!NON_LOGGABLE_TYPES.contains(message.runtimeType)) {
      Logger.debug(() {
        ...
        } else if (message is ProtoOAGetAccountListByAccessTokenReq ||
            message is ProtoOAAccountAuthReq) {
          payload['accessToken'] = '***';
        }
  ...
  Future<T> _query<T extends ProtoMessage>(ProtoMessage message) {
    ...
    _waiters[id] =
        RemoteAPIWaiter(DateTime.now().millisecondsSinceEpoch, completer);
  ...
  void _handleSocketMessage(List<int> data) {
    ...
    if (constructor != null) {
      ...
      if (json['clientMsgId'] != null)
        message.cmdId = json['clientMsgId'] as String;

      if (message is ProtoOAErrorRes) {
        Logger.log(
            () => '$logTag Server error ${jsonEncode(message.$payload())}');
      ...
      Logger.log(() =>
          '$logTag unhandeled message: $payloadType\n${jsonEncode(json["payload"])}');
    }
  }

  void _handleHeartBeatEvent(ProtoHeartbeatEvent event) =>
      _send(ProtoHeartbeatEvent());
  ...
  void _notifyListeners(ProtoMessage message) {
    ...
    for (final Function listener in _listeners[message.runtimeType]!) {
      try {
        ...
        Logger.error('$logTag error occurred in excecuting listener.', e,
            StackTrace.current);
      }
    }
  }
  ...
  void _notifyOneShotWaiters(ProtoMessage message) {
    if (message.cmdId == null || !_oneShotWaiters.containsKey(message.cmdId))
      return;
  ...
  void _checkWaitersForTimeout() {
    ...
    final int extreamTimestamp =
        DateTime.now().millisecondsSinceEpoch - requestTimeout.inMilliseconds;
  ...
  void _cancelAllWaiters(String reason) {
    for (final String key in _waiters.keys) {
      try {
        ...
        Logger.error(
            '$logTag error occurred at canceling waiter with error', e);
      }
    }
    _waiters.clear();
    ...
    for (final String cmdId in _oneShotWaiters.keys) {
      for (final RemoteAPIWaiter waiter in _oneShotWaiters[cmdId]!) {
        try {
          ...
          Logger.error(
              '$logTag error occurred at canceling oneshot waiter with error',
              e);
        }
      }
    }
    ...
    for (final Completer<void> waiter in _appAutorizationWaiters) {
      try {
        ...
        Logger.error(
            '$logTag error occurred at canceling app autorization waiter with error',
            err);
      }
    }
  ...
  Future<void> connect() async {
    try {
      ...
      Logger.log(() =>
          '$logTag socket connected to $url(${_socket!.remoteAddress.host})');
      ...
      _socket!.handleError(
          (Object err) => Logger.error('$logTag socket error', err));
    } catch (e) {
    ...
    try {
      ...
      Logger.error(
          '$logTag Error occurred at aplication authenthification process',
          err);
      rethrow;
    }
  }
  ...
  Future<void> reconnect() async {
    ...
    try {
      final Iterable<TraderData> traders = isDemo
          ? GetIt.I<UserState>().demoTraders
          : GetIt.I<UserState>().liveTraders;
  ...
  Future<ProtoMessage> waitForResponse(String msgId) {
    if (!_oneShotWaiters.containsKey(msgId))
      _oneShotWaiters[msgId] = Queue<RemoteAPIWaiter>();

    final RemoteAPIWaiter waiter = RemoteAPIWaiter(
        DateTime.now().millisecondsSinceEpoch, Completer<ProtoMessage>());
  ...
  Future<ProtoOAApplicationAuthRes> _autorizeApplication() async {
    ...
    final ProtoOAApplicationAuthRes resp = await _query<
            ProtoOAApplicationAuthRes>(
        ProtoOAApplicationAuthReq(appState.clientID, appState.clientSecret));

    while (_appAutorizationWaiters.isNotEmpty)
      _appAutorizationWaiters.removeFirst().complete();

    return resp;
  }

  // REQUEST REQUIRING APPLICATION AUTORIZATION
  Future<ProtoOAGetAccountListByAccessTokenRes> getAccountListByAccessTokenReq(
      String token) async {
    await whenAutorized();

    return _query<ProtoOAGetAccountListByAccessTokenRes>(
        ProtoOAGetAccountListByAccessTokenReq(token));
  }
  ...
  Future<ProtoOAAccountAuthRes> autorizeAccount(int accountId) async {
    ...
    if (token == null)
      return Future<ProtoOAAccountAuthRes>.error('NO_ACCESS_TOKEN_FOUND');

    final ProtoOAAccountAuthRes resp = await _query<ProtoOAAccountAuthRes>(
        ProtoOAAccountAuthReq(accountId, token));
    _autorizedAccounts.add(accountId);

    final Queue<Completer<void>>? waiters =
        _accountAutorizationWaiters[accountId];
    if (waiters != null)
      while (waiters.isNotEmpty) waiters.removeFirst().complete();
  ...
  Future<ProtoOAAssetClassListRes> sendAssetClassListReq(int accountId) async {
    ...
    return _query<ProtoOAAssetClassListRes>(
        ProtoOAAssetClassListReq(accountId));
  }
  ...
  Future<ProtoOASymbolCategoryListRes> sendSymbolCategoryListReq(
      int accountId) async {
    await whenAccountAutorized(accountId: accountId);

    return _query<ProtoOASymbolCategoryListRes>(
        ProtoOASymbolCategoryListReq(accountId));
  }
  ...
  Future<ProtoOASymbolByIdRes> sendSymbolById(
      int accountId, List<int> symbolIds) async {
    await whenAccountAutorized(accountId: accountId);

    return _query<ProtoOASymbolByIdRes>(
        ProtoOASymbolByIdReq(accountId, symbolIds));
  }

  Future<ProtoOAGetDynamicLeverageByIDRes> sendLeverageById(
      int accountId, int leverageId) async {
    await whenAccountAutorized(accountId: accountId);

    return _query<ProtoOAGetDynamicLeverageByIDRes>(
        ProtoOAGetDynamicLeverageByIDReq(accountId, leverageId));
  }

  Future<ProtoOAGetTrendbarsRes> sendGetTrendbars(int accountId, int symbolId,
      int count, ProtoOATrendbarPeriod period, int fromTsMs, int toTsMs) async {
    await whenAccountAutorized(accountId: accountId);

    return _query<ProtoOAGetTrendbarsRes>(ProtoOAGetTrendbarsReq(
        accountId, fromTsMs, toTsMs, period, symbolId, count));
  }

  Future<ProtoOASubscribeSpotsRes> subscribeForSpots(
      int accountId, List<int> symbolsIds) async {
    await whenAccountAutorized(accountId: accountId);

    return _query<ProtoOASubscribeSpotsRes>(
        ProtoOASubscribeSpotsReq(accountId, symbolsIds));
  }

  Future<ProtoOAUnsubscribeSpotsRes> unsubscribeFromSpots(
      int accountId, List<int> symbolsIds) async {
    await whenAccountAutorized(accountId: accountId);
  ...
  Future<ProtoOASubscribeLiveTrendbarRes> subscribeForLiveTrendbars(
      int accountId, int symbolId, ProtoOATrendbarPeriod period) async {
    await whenAccountAutorized(accountId: accountId);

    return _query<ProtoOASubscribeLiveTrendbarRes>(
        ProtoOASubscribeLiveTrendbarReq(accountId, period, symbolId));
  }

  Future<ProtoOAUnsubscribeLiveTrendbarRes> unsubscribeFromLiveTrendbars(
      int accountId, int symbolId, ProtoOATrendbarPeriod period) async {
    await whenAccountAutorized(accountId: accountId);

    return _query<ProtoOAUnsubscribeLiveTrendbarRes>(
        ProtoOAUnsubscribeLiveTrendbarReq(accountId, period, symbolId));
  }

  Future<ProtoOAExpectedMarginRes> sendExpectedMargin(
      int accountId, int symbolId, List<int> volumes) async {
    await whenAccountAutorized(accountId: accountId);

    return _query<ProtoOAExpectedMarginRes>(
        ProtoOAExpectedMarginReq(accountId, symbolId, volumes));
  }
  ...
  Future<ProtoOAGetPositionUnrealizedPnLRes> sendGetPositionsPnl(
      int accountId) async {
    await whenAccountAutorized(accountId: accountId);

    return _query<ProtoOAGetPositionUnrealizedPnLRes>(
        ProtoOAGetPositionUnrealizedPnLReq(accountId));
  }

  Future<ProtoMessage> sendClosePosition(
      int accountId, int positionId, int volume) async {
    await whenAccountAutorized(accountId: accountId);
  ...
  Future<ProtoMessage> sendEditOrder(
    ...
    final ProtoOAAmendOrderReq req =
        ProtoOAAmendOrderReq(accountId, orderId, volume)
  ...
  Future<ProtoOADealListRes> sendDealList(
      int accountId, int fromTsSec, int toTsSec) async {
    await whenAccountAutorized(accountId: accountId);

    return _query<ProtoOADealListRes>(
        ProtoOADealListReq(accountId, fromTsSec, toTsSec));
  }

  Future<ProtoOADealOffsetListRes> sendDealOffsetList(
      int accountId, int dealId) async {
    await whenAccountAutorized(accountId: accountId);

    return _query<ProtoOADealOffsetListRes>(
        ProtoOADealOffsetListReq(accountId, dealId));
  }

  Future<ProtoOASymbolsForConversionRes> sendSymbolsForConversion(
      int accountId, int firstAssetId, int lastAssetId) {
    return _query(
        ProtoOASymbolsForConversionReq(accountId, firstAssetId, lastAssetId));
  }
}
