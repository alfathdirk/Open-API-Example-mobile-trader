...
abstract class ISubscriber {
  ...
  void finishWithError(Object error) {
    try {
      ...
      Logger.error(
          'Error occurred at execution of subscriber with fail result', err);
    }
  }
}
...
class BarSubscriber extends ISubscriber {
  BarSubscriber(super.completer, this.traderId, this.symbolId, this.period,
      this.subscribe);
...
class SpotSubscriptionManager {
  ...
  final List<SubscribedSymbolBars> _subscribedSymbolBars =
      <SubscribedSymbolBars>[];
  final Queue<BarSubscriber> _barsQueue = Queue<BarSubscriber>();
  bool _isWaiting = false;
  List<int> _cachedSubscribedSymbols = <int>[];
  List<SubscribedSymbolBars> _cachedSubscribedSymbolBars =
      <SubscribedSymbolBars>[];

  Iterable<int> get subscribedSymbols => _subscribedSymbols;

  bool isSymbolSubscribedForSpots(int symbolId) =>
      _subscribedSymbols.contains(symbolId);
  bool isSymbolSubscribedForBars(int symbolId) => _subscribedSymbolBars
      .any((SubscribedSymbolBars sub) => sub.symbolid == symbolId);
  ...
  void _nextSpotsAction() {
    ...
    if (subscriber.subscribe) {
      final Iterable<int> symbols = subscriber.symbols
          .where((int symbol) => !_subscribedSymbols.contains(symbol));
      if (symbols.isEmpty) {
        subscriber.finish();
        _isWaiting = false;
        _nextSpotsActionAsync();
      ...
      _subscribeForSpotsOnServer(subscriber.traderId, symbols.toList())
      ...
      _remoteApi
          .unsubscribeFromSpots(subscriber.traderId, subscriber.symbols)
          .then((ProtoOAUnsubscribeSpotsRes resp) {
  ...
  Future<void> _subscribeForSpotsOnServer(int traderId, List<int> symbols) {
    ...
    _remoteApi.subscribeForSpots(traderId, symbols).then(
        (ProtoOASubscribeSpotsRes resp) {
          ...
          for (final int s in symbols) {
            try {
              ...
              if (err is ProtoOAErrorRes &&
                  err.errorCode == 'ALREADY_SUBSCRIBED') {
                _subscribedSymbols.add(s);
              } else {
  ...
  Future<void> _nextBarsAction() async {
    ...
    if (subscriber.subscribe) {
      ...
      if (_subscribedSymbolBars
          .where((SubscribedSymbolBars element) =>
              element.symbolid == subscriber.symbolId &&
              element.period == subscriber.period)
          .isNotEmpty) {
        subscriber.finish();
      } else {
        try {
          await _remoteApi.subscribeForLiveTrendbars(
              subscriber.traderId, subscriber.symbolId, subscriber.period);
          _subscribedSymbolBars.add(
              SubscribedSymbolBars(subscriber.symbolId, subscriber.period));
          subscriber.finish();
        } catch (err) {
          if (err is ProtoOAErrorRes && err.errorCode == 'ALREADY_SUBSCRIBED') {
            _subscribedSymbolBars.add(
      ...
      try {
        await _remoteApi.unsubscribeFromLiveTrendbars(
            subscriber.traderId, subscriber.symbolId, subscriber.period);
        _subscribedSymbolBars.removeWhere((SubscribedSymbolBars sub) =>
            sub.symbolid == subscriber.symbolId &&
            sub.period == subscriber.period);
        subscriber.finish();
      } catch (err) {
  ...
  Future<void> _subscribeForLiveTrendBarsOnServer(
      int traderId, List<SubscribedSymbolBars> symbols) async {
    if (symbols.isEmpty) return Future<void>.value();
    ...
    for (int i = symbols.length - 1; i >= 0; i--) {
      ...
      try {
        await _remoteApi.subscribeForLiveTrendbars(
            traderId, data.symbolid, data.period);
      } on ProtoOAErrorRes catch (err) {
        if (err.errorCode != 'ALREADY_SUBSCRIBED') {
          Logger.log(() =>
              'Error occurred at subscriprtion to live trenbars of symbol(${data.symbolid}) for ${data.period.name}(${data.period.index})');
  ...
  Future<void> subscribeForTrendBars(
      int traderId, int symbolId, ProtoOATrendbarPeriod period) {
    final Completer<void> c = Completer<void>();
  ...
  Future<void> unsubscribeFromTrendBars(
      int traderId, int symbolId, ProtoOATrendbarPeriod period) {
    final Completer<void> c = Completer<void>();
  ...
  Future<void> restoreSubscriptions() async {
    ...
    await _subscribeForLiveTrendBarsOnServer(
        traderId, _cachedSubscribedSymbolBars);
  }
}
