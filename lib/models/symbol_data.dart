...
import 'package:myfxhistory_app/models/pair.dart';
...
class SymbolData {
  ...
  static double humanicRateFromSystem(int rate) =>
      rate / _SYSTEM_CONVERTION_RATE;
  static int systemRateFromHumanic(double rate) =>
      (rate * _SYSTEM_CONVERTION_RATE).round();
  ...
  static String formattedVolumeDefault({int? system, double? humanic}) {
    ...
    return humanic.toComaSeparated(
        decimals: system % 100 == 0 ? 0 : (system % 10 == 0 ? 1 : 2));
  }
  ...
  String get formattedSellRate =>
      details?.formattedRate(system: bid) ??
      SymbolData.formattedRateDefault(system: bid);
  String get formattedBuyRate =>
      details?.formattedRate(system: ask) ??
      SymbolData.formattedRateDefault(system: ask);
  String get formattedDailyChangePips => (dailyChangePips / 100000).toString();
  String get formattedDailyChangePercents =>
      '${(dailyChangePercents * 100).toStringAsFixed(2)}%';
  String get units =>
      details?.data.measurementUnits ?? baseAsset?.units ?? 'n/a';
  ...
  int get stopLossDistance {
    ...
    final int slDistance = trader.isLimitedRisk
        ? (details!.data.gslDistance ?? 0)
        : (details!.data.slDistance ?? details!.data.gslDistance ?? 0);
    int distance;

    if (details!.data.distanceSetIn ==
        proto.ProtoOASymbolDistanceType.symbolDistanceInPercentage) {
    ...
    return details!
        .cutOffExtraDigitsFromRate(max(distance, 5 * pipSize), false);
  }
  ...
  int get takeProfitDisatance {
    ...
    if (details!.data.tpDistance != null) {
      if (details!.data.distanceSetIn ==
        ...
        final int rate = (((ask ?? 0) + (bid ?? 0)) / 2).round();
        distance =
            (rate * details!.data.tpDistance! / 10000 / pip).round() * pip;
      } else {
  ...
  List<Pair<tz.TZDateTime, tz.TZDateTime>> scheduleInDates() {
    final List<Pair<tz.TZDateTime, tz.TZDateTime>> result =
        <Pair<tz.TZDateTime, tz.TZDateTime>>[];
    ...
    if (details?.data.schedule != null) {
      ...
      for (final proto.ProtoOAInterval interval in details!.data.schedule!) {
        ...
        result.add(
            Pair<tz.TZDateTime, tz.TZDateTime>(start.toLocal(), end.toLocal()));
      }
    }
  ...
  bool isMarketClosed() {
    ...
    final List<proto.ProtoOAHoliday> holidays =
        details?.holidays ?? <proto.ProtoOAHoliday>[];
  ...
  Future<SymbolDetailsData> getDetailsData() async {
    ...
    await GetIt.I<RemoteAPIManager>()
        .getAPI(demo: trader.demo)
        .sendSymbolById(trader.id, <int>[id]);
    return details!;
  }
...
class SymbolDetailsData {
  ...
  List<proto.ProtoOADynamicLeverageTier>? get leverages =>
      symbol.category.assetClass.tree.leveragesBy(data.leverageId);
  ...
  List<proto.ProtoOAHoliday>? get holidays {
    ...
    final DateTime now = DateTime.now();
    final Iterable<proto.ProtoOAHoliday> holidays =
        data.holiday!.where((proto.ProtoOAHoliday holiday) {
    ...
    _holidays!.sort((proto.ProtoOAHoliday a, proto.ProtoOAHoliday b) =>
  ...
  String formattedVolume({int? system, double? humanic}) {
    ...
    return humanic.toComaSeparated(
        decimals: vol % 100 == 0 ? 0 : (vol % 10 == 0 ? 1 : 2));
  }

  String formattedVolumeWithUnits({int? system, double? humanic}) {
    return formattedVolume(system: system, humanic: humanic) +
        ' ' +
        (data.measurementUnits ?? symbol.baseAsset?.units ?? '');
  }
}
