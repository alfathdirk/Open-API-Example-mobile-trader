...
class ActivityManager<T extends OrderPosDataBase> {
  ...
  final Map<ActivityManagerEventType, List<Function(T)>> _listeners =
      <ActivityManagerEventType, List<Function(T)>>{};
  ...
  void _dispatchEvent(ActivityManagerEventType type, T activity) {
    for (final Function(T) l in _listeners[type] ?? <Function(T)>[]) {
      try {
        ...
        Logger.error(
            'Error occurred at executing listener of activity event', e);
      }
    }
  }
