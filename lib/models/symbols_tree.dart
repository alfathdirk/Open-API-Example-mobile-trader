...
class SymbolsTree {
  ...
  final Map<int, List<proto.ProtoOADynamicLeverageTier>> _leverages =
      <int, List<proto.ProtoOADynamicLeverageTier>>{};
  final Set<int> _collapsedCategoris = <int>{};

  Iterable<AssetClassData> get assetClasses => _classes.values;

  AssetClassData? assetClass(int? id) =>
      _classes.containsKey(id) ? _classes[id] : null;
  SymbolCategoryData? symbolCategory(int? id) {
    if (id != null &&
        _categories.any((SymbolCategoryData cat) => cat.id == id)) {
  ...
  SymbolData? symbolByAssetPair(int firstAssetId, int secondAssetId) {
    for (final SymbolData symbol in _symbols.values) {
      if (symbol.baseAssetId == firstAssetId &&
          symbol.quoteAssetId == secondAssetId) return symbol;
      if (symbol.baseAssetId == secondAssetId &&
          symbol.quoteAssetId == firstAssetId) return symbol;
    }
  ...
  void handleSymbolCategoryResponse(
      List<proto.ProtoOASymbolCategory> categories) {
    _categories.clear();

    categories
        .sort((proto.ProtoOASymbolCategory a, proto.ProtoOASymbolCategory b) {
  ...
  void toggleFavoriteSymbol(int symbolId) => isFavorite(symbolId)
      ? removeFavoriteSymbol(symbolId)
      : addFavoriteSymbol(symbolId);

  bool hasLeverage(int leverageId) => _leverages.containsKey(leverageId);
  void addLeverage(proto.ProtoOADynamicLeverage leverage) =>
      _leverages[leverage.leverageId] = leverage.tiers;
  List<proto.ProtoOADynamicLeverageTier>? leveragesBy(int? id) =>
      _leverages[id];

  Iterable<int> get collapsedCategories => _collapsedCategoris;
  bool isCategoryCollapsed(int? id) => _collapsedCategoris.contains(id);
  void collapseCategory(int? id) =>
      id != null ? _collapsedCategoris.add(id) : null;
  void expandCategory(int? id) =>
      id != null ? _collapsedCategoris.remove(id) : null;
  void toggleCategory(int? id) =>
      isCategoryCollapsed(id) ? expandCategory(id) : collapseCategory(id);
  void restoreCollapsedCategories(Iterable<int> ids) =>
      _collapsedCategoris.addAll(ids);

  Iterable<SymbolData> findSymbolForSimultaneousTrading(
    ...
    if (name.length > 5) {
      ...
      regExp = RegExp(
          r'^' + name.substring(0, 3) + '/' + name.substring(3, 6) + r'$');
      for (final SymbolData symbol in _symbols.values) {
        if (regExp.hasMatch(symbol.name)) result.add(symbol);
      }
...
class AssetClassData {
  ...
  bool get hasEnabledSymbols =>
      categories.any((SymbolCategoryData c) => c.hasEnabledSymbols);
  List<SymbolData> get symbols => categories.fold(<SymbolData>[],
          (List<SymbolData> prev, SymbolCategoryData cat) {
