...
abstract class OrderPosDataBase {
  ...
  String get formattedVolume {
    return symbol?.details?.formattedVolume(system: volume) ??
        SymbolData.formattedVolumeDefault(system: volume);
  }

  String get formattedVolumeWithUnits {
    if (measurementUnits.isNotEmpty)
      return '$formattedVolume $measurementUnits';
    return symbol?.details?.formattedVolumeWithUnits(system: volume) ??
        formattedVolume;
  }
  ...
  String formattedRate({int? system, double? humanic}) {
    ...
    if (symbol?.details != null)
      return symbol!.details!.formattedRate(system: system, humanic: humanic);
  ...
  void updateTimestamp([int? ts]) => _updated =
