...
class TraderData {
  ...
  final ActivityManager<PositionData> positionsManager =
      ActivityManager<PositionData>();
  final ActivityManager<OrderData> ordersManager = ActivityManager<OrderData>();

  proto.ProtoOAAccountType get accountType =>
      _respTrader.accountType ?? proto.ProtoOAAccountType.spreadBetting;
  proto.ProtoOAAccessRights get accessRights => _respTrader.accessRights;
  proto.ProtoOALimitedRiskMarginCalculationStrategy?
      get limitedRiskMarginCalculationStrategy =>
          _respTrader.limitedRiskMarginCalculationStrategy;
...
// Update it to calculations of pnl updates
  ...
  String formattedMoney({int? cents, double? money}) =>
      (money ?? toMoney(cents ?? 0)).toComaSeparated(decimals: decimals);
  String formattedMoneyWithCurrency({int? cents, double? money}) =>
      formattedMoney(cents: cents, money: money) + ' $currencySign';

  String get formattedMarginLevel =>
      margin != 0 ? marginLevel.toComaSeparated(decimals: 2) : '----';

  RemoteApi get remoteApi => GetIt.I<RemoteAPIManager>().getAPI(demo: demo);
  SpotSubscriptionManager get subscriptionManagerApi =>
      GetIt.I<RemoteAPIManager>().getSpotSubscriptionManager(demo: demo);
  ...
  void recalculateFields() {
    ...
    for (final PositionData position in positionsManager.activities) {
      ...
      marginsBySymbol[position.symbolId]![position.isBuy ? 0 : 1] +=
          position.usedMargin;
    ...
    if (isLimitedRisk &&
        limitedRiskMarginCalculationStrategy !=
            proto.ProtoOALimitedRiskMarginCalculationStrategy
                .accordingToLeverage) {
      margin = marginsBySymbol.values.fold(
          0, (int total, List<int> margins) => total + margins[0] + margins[1]);
    } else {
      margin = switch (_respTrader.totalMarginCalculationType) {
        proto.ProtoOATotalMarginCalculationType.max => marginsBySymbol.values
            .fold(
                0,
                (int total, List<int> margins) =>
                    total + math.max(margins[0], margins[1])),
        proto.ProtoOATotalMarginCalculationType.sum => marginsBySymbol.values
            .fold(
                0,
                (int total, List<int> margins) =>
                    total + margins[0] + margins[1]),
        proto.ProtoOATotalMarginCalculationType.net => marginsBySymbol.values
            .fold(
                0,
                (int total, List<int> margins) =>
                    total + (margins[0] - margins[1]).abs()),
      };
    }
  }
  ...
  void handleMarginChangedEvent(proto.ProtoOAMarginChangedEvent event) {
    positionsManager.activityBy(id: event.positionId)?.usedMargin =
        event.usedMargin;
  ...
  void removePosition(int id, proto.ProtoOAClosePositionDetail? details) {
    if (positionsManager.removeActivity(id)) {
      if (details != null)
        updateBalance(details.balanceVersion ?? 0, details.balance);
      recalculateFields();
    }
  }
  ...
  void updatePositionTrailingStop(proto.ProtoOATrailingSLChangedEvent event) {
    positionsManager
        .activityBy(id: event.positionId)
        ?.updateTrailingStopValue(event.stopPrice);
  }
