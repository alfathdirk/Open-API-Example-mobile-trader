...
extension DoubleExtention on double {
  ...
  String toComaSeparated({int? decimals}) {
    ...
    buffer.write(integer.toString().replaceAllMapped(
        RegExp(r'(\d)(?=(\d{3})+(?!\d))'),
...
extension DateTimeExtention on DateTime {
  ...
  String formatted([String format = 'HH:mm:ss, d MMMM y']) =>
...
extension ProtoOAHolidayExtention on ProtoOAHoliday {
  ...
  DateTime startDateTime() {
    ...
    return tz.TZDateTime(location, 1970, DateTime.january, holidayDate + 1, 0,
            0, startSecond ?? 0)
        .toLocal();
  }
...
extension StringExtension on String {
  String centerLabel(int width, [String symbol = ' ']) =>
      padLeft(width ~/ 2 + length ~/ 2, symbol).padRight(width, symbol);
}
